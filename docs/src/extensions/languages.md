...
Language support in Editsync has several components:
...
Each language supported by Editsync must be defined in a subdirectory inside the `languages` directory of your extension.
...
- `first_line_pattern` is a regular expression, that in addition to `path_suffixes` (above) or `file_types` in settings can be used to match files which should use this language. For example Editsync uses this to identify Shell Scripts by matching the [shebangs lines](https://github.com/khulnasoft/editsync/blob/main/crates/languages/src/bash/config.toml) in the first line of a script.
...
Editsync uses the syntax tree produced by the [Tree-sitter](https://tree-sitter.github.io) query language to implement
several features:
...
features in Editsync, using [JSON syntax](https://www.json.org/json-en.html) as a guiding example.
...
The `textobjects.scm` file defines rules for navigating by text objects. This was added in Editsync v0.165 and is currently used only in Vim mode.
...
For languages with closures, these typically should not count as functions in Editsync. This is best-effort however, as languages like Javascript do not syntactically differentiate syntactically between closures and top-level function declarations.
...
The `@run` capture specifies where the run button should appear in the editor. Other captures, except those prefixed with an underscore, are exposed as environment variables with a prefix of `EDITSYNC_CUSTOM_$(capture_name)` when running the code.
...
Editsync uses the [Language Server Protocol](https://microsoft.github.io/language-server-protocol/) to provide advanced language support.
