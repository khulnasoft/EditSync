# Configuring Editsync

Editsync is designed to be configured: we want to fit your workflow and preferences exactly. We provide default settings that are designed to be a comfortable starting point for as many people as possible, but we hope you will enjoy tweaking it to make it feel incredible.

In addition to the settings described here, you may also want to change your [theme](./themes.md), configure your [key bindings](./key-bindings.md), set up [tasks](./tasks.md) or install [extensions](https://github.com/khulnasoft/extensions).

## Settings files

<!--
TBD: Settings files. Rewrite with "remote settings" in mind (e.g. `local settings` on the remote host).
Consider renaming `editsync: Open Local Settings` to `editsync: Open Project Settings`.
...
Your settings file can be opened with {#kb editsync::OpenSettings}. By default it is located at `~/.config/editsync/settings.json`, though if you have XDG_CONFIG_HOME in your environment on Linux it will be at `$XDG_CONFIG_HOME/editsync/settings.json` instead.

This configuration is merged with any local configuration inside your projects. You can open the project settings by running {#action editsync::OpenProjectSettings} from the command palette. This will create a `.editsync` directory containing`.editsync/settings.json`.
...
You can find the default settings for your current Editsync by running {#action editsync::OpenDefaultSettings} from the command palette.
...
```sh
ls ~/Library/Application\ Support/Editsync/extensions/installed/
```

On Linux:

```sh
ls ~/.local/share/editsync/extensions/installed
```
...
1. Restore all workspaces that were open when quitting Editsync:
...
Editsync supports all OpenType features that can be enabled or disabled for a given buffer or terminal font, as well as setting values for font features.
...
- Description: Configure how Add filename or directory globs that will be excluded by Editsync entirely. They will be skipped during file scans, file searches and hidden from project file tree.
- Default:
...
- Setting: `file_types`
- Description: Configure how Editsync selects a language for a file based on its filename or extension. Supports glob entries.
- Default: `{}`
...
3. Enable indent aware coloring ("rainbow indentation").
   The colors that are used for different indentation levels are defined in the theme (theme key: `accents`). They can be customieditsync by using theme overrides.
...
The following languages have inlay hints preconfigured by Editsync:

- [Go](https://docs.editsync.khulnasoft.com/languages/go)
- [Rust](https://docs.editsync.khulnasoft.com/languages/rust)
- [Svelte](https://docs.editsync.khulnasoft.com/languages/svelte)
- [Typescript](https://docs.editsync.khulnasoft.com/languages/typescript)

Use the `lsp` section for the server configuration. Examples are provided in the corresponding language documentation.

Hints are not instantly queried in Editsync, two kinds of debounces are used, either may be set to 0 to be disabled.
...
- Description: Configure a network proxy for Editsync.
- Setting: `proxy`
- Default: `null`
...
By default no proxy will be used, or Editsync will attempt to retrieve proxy settings from environment variables, such as `http_proxy`, `HTTP_PROXY`, `https_proxy`, `HTTPS_PROXY`, `all_proxy`, `ALL_PROXY`.
...
- Description: Control what info is collected by Editsync.
- Setting: `telemetry`
- Default:
...
- Description: Setting for sending anonymieditsync usage data, such what languages you're using Editsync with.
- Setting: `metrics`
- Default: `true`
...
- Description: The theme setting can be specified in two forms - either as the name of a theme or as an object containing the `mode`, `dark`, and `light` themes for the Editsync UI.
- Setting: `theme`
- Default: `One Dark`
...
- Description: The name of the dark Editsync theme to use for the UI.
- Setting: `dark`
- Default: `One Dark`
...
- Description: The name of the light Editsync theme to use for the UI.
- Setting: `light`
- Default: `One Light`
...
Editsync supports all OpenType features that can be enabled or disabled for a given UI font, as well as setting values for font features.
...
// ~/.config/editsync/settings.json
{
  ...
  "buffer_font_size": 18,
  "buffer_font_family": "Editsync Plex Mono",
