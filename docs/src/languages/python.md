...
Python support is available natively in Editsync.
...
A common tool for formatting Python code is [Ruff](https://docs.astral.sh/ruff/). It is another tool written in Rust, an extremely fast Python linter and code formatter. It is available through the [Ruff extension](https://github.com/khulnasoft/editsync/tree/main/extensions/ruff/). To configure the Ruff extension to work within Editsync, see the setup documentation [here](https://docs.astral.sh/ruff/editors/setup/#editsync).

## Virtual Environments in the Terminal {#terminal-detect_venv}

Editsync will also detect virtual environments and automatically activate them in terminal if available.
See: [detect_venv documentation](../configuring-editsync.md#terminal-detect_venv) for more.
