...
Rust support is available natively in Editsync.
...
<!--
TBD: Polish Rust Docs. Editsync is a good rust editor, good Rust docs make it look like we care about Rust (we do!)
...
You can configure which `rust-analyzer` binary Editsync should use.

By default, Editsync will try to find a `rust-analyzer` in your `$PATH` and try to use that. If that binary successfully executes `rust-analyzer --help`, it's used. Otherwise, Editsync will fall back to installing its own `rust-analyzer` version and using that.

If you want to disable Editsync looking for a `rust-analyzer` binary, you can set `ignore_system_version` to `true` in your `settings.json`:
...
<!--
TBD: Is it possible to specify RUSTFLAGS? https://github.com/khulnasoft/editsync/issues/14334
-->
...
Rust-analyzer in Editsync runs with the default parameters.
...
Which would mean that every time Editsync saves, a `cargo check --workspace --all-targets` command is run, checking the entire project (workspace), lib, doc, test, bin, bench and [other targets](https://doc.rust-lang.org/cargo/reference/cargo-targets.html).

While that works fine on small projects, it does not scale well.

The alternatives would be to use [tasks](../tasks.md), as Editsync already provides a `cargo check --workspace --all-targets` task and the ability to cmd/ctrl-click on the terminal output to navigate to the error, and limit or turn off the check on save feature entirely.
...
Here's a snippet for Editsync settings.json (the language server will restart automatically after the `lsp.rust-analyzer` section is edited and saved):
