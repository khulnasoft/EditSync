...
Support for Ansible in Editsync is provided via a community-maintained [Ansible extension](https://github.com/kartikvashistha/editsync-ansible).
...
By default, to avoid mishandling non-Ansible YAML files, the Ansible Language is not associated with any file extensions by default. To change this behavior you can add a `"file_types"` section to the Editsync settings inside your project (`.editsync/settings.json`) or your Editsync user settings (`~/.config/editsync/settings.json`) to match your folder/naming conventions. For example:
...
LSP options for this extension can be configured under Editsync's settings file. To get the best experience, add the following configuration under the `"lsp"` section in your `~/.editsync/settings.json`:

```json
"lsp": {
  // Note, the Editsync Ansible extension prefixes all settings with `ansible`
