...
use editsync_extension_api::{self as editsync, http_client::RedirectPolicy, Result};

struct Perplexity;

impl editsync::Extension for Perplexity {
    fn new() -> Self {
        Self
    }
    ...
    fn run_slash_command(
        &self,
        command: editsync::SlashCommand,
        argument: Vec<String>,
        worktree: Option<&editsync::Worktree>,
    ) -> editsync::Result<editsync::SlashCommandOutput> {
        ...
        if query.is_empty() {
            return Ok(editsync::SlashCommandOutput {
        ...
        match editsync::http_client::fetch_stream(&request) {
            Ok(stream) => {
                ...
                Ok(editsync::SlashCommandOutput {
                    text: full_content,
                    sections: vec![],
                })
            }
            Err(e) => Ok(editsync::SlashCommandOutput {
    ...
    fn complete_slash_command_argument(
        &self,
        _command: editsync::SlashCommand,
        query: Vec<String>,
    ) -> editsync::Result<Vec<editsync::SlashCommandArgumentCompletion>> {
        let suggestions = vec!["How do I develop a Editsync extension?"];
        let query = query.join(" ").to_lowercase();

        Ok(suggestions
            .into_iter()
            .filter(|suggestion| suggestion.to_lowercase().contains(&query))
            .map(|suggestion| editsync::SlashCommandArgumentCompletion {
    ...
    fn language_server_command(
        &mut self,
        _language_server_id: &editsync_extension_api::LanguageServerId,
        _worktree: &editsync_extension_api::Worktree,
    ) -> Result<editsync_extension_api::Command> {
...
editsync::register_extension!(Perplexity);
