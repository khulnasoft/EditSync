...
impl ElixirLs {
    ...
    pub fn language_server_binary_path(
        ...
        let (platform, _arch) = editsync::current_platform();
        let version_dir = format!("elixir-ls-{}", release.version);
        let extension = match platform {
            editsync::Os::Mac | editsync::Os::Linux => "sh",
            editsync::Os::Windows => "bat",
        };
        ...
        if !fs::metadata(&binary_path).map_or(false, |stat| stat.is_file()) {
            ...
            editsync::make_file_executable(&format!("{version_dir}/launch.{extension}"))?;
            editsync::make_file_executable(&format!("{version_dir}/debug_adapter.{extension}"))?;
