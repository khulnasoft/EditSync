...
impl AstroExtension {
    ...
    fn typescript_exists_for_worktree(&self, worktree: &editsync::Worktree) -> Result<bool> {
    ...
    fn install_typescript_if_needed(&mut self, worktree: &editsync::Worktree) -> Result<()> {
        if self
        ...
        if installed_typescript_version.as_ref() != Some(&latest_typescript_version) {
            ...
            editsync::npm_install_package(TYPESCRIPT_PACKAGE_NAME, &latest_typescript_version)?;
        } else {
...
impl editsync::Extension for AstroExtension {
    fn new() -> Self {
        Self {
...
editsync::register_extension!(AstroExtension);
