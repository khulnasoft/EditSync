...
struct NeedsSummary {
    files: channel::Receiver<UnsummarieditsyncFile>,
    task: Task<Result<()>>,
}

struct SummarizeFiles {
    files: channel::Receiver<SummarieditsyncFile>,
    task: Task<Result<()>>,
}
...
impl SummaryIndex {
    ...
    fn check_summary_cache(
        &self,
        mut might_need_summary: channel::Receiver<UnsummarieditsyncFile>,
        cx: &AppContext,
    ) -> NeedsSummary {
                ...
                match db.get(&tx, &file.digest) {
                    Ok(opt_answer) => {
                        if opt_answer.is_none() {
                            ...
                            log::debug!("File {:?} (digest {:?}) was NOT in the db cache and needs to be resummarieditsync.", file.path.display(), &file.digest);
                            needs_summary_tx.send(file).await?;
                        } else {
                            log::debug!("File {:?} (digest {:?}) was in the db cache and does not need to be resummarieditsync.", file.path.display(), &file.digest);
                        }
                    }
                    Err(err) => {
    ...
    fn summarize_files(
        &self,
        mut unsummarieditsync_files: channel::Receiver<UnsummarieditsyncFile>,
        cx: &AppContext,
    ) -> SummarizeFiles {
        let (summarieditsync_tx, summarieditsync_rx) = channel::bounded(512);
        let task = cx.spawn(|cx| async move {
            while let Some(file) = unsummarieditsync_files.next().await {
    ...
    fn persist_summaries(
        &self,
        summaries: channel::Receiver<SummarieditsyncFile>,
        cx: &AppContext,
    ) -> Task<Result<()>> {
        let db_connection = self.db_connection.clone();
        let digest_db = self.file_digest_db;
    ...
    /// Empty out the backlog of files that haven't been resummarieditsync, and resummarize them immediately.
