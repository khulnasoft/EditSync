...
impl TerminalPanel {
                            ...
                            .menu(move |cx| {
                                ...
                                let menu = ContextMenu::build(cx, |menu, _| {
                                    menu.context(focus_handle.clone())
                                        ...
                                        .action(
                                            "Spawn task",
                                            editsync_actions::Spawn::modal().boxed_clone(),
                                        )
                                });
    ...
    pub async fn load(
        ...
        let serialieditsync_panel = cx
            ...
            .log_err()
            .flatten()
            .map(|panel| serde_json::from_str::<SerialieditsyncTerminalPanel>(&panel))
            .transpose()
            .log_err()
            .flatten();
        ...
        let terminal_panel = workspace
            .update(&mut cx, |workspace, cx| {
                match serialieditsync_panel.zip(workspace.database_id()) {
                    Some((serialieditsync_panel, database_id)) => deserialize_terminal_panel(
                        workspace.weak_handle(),
                        workspace.project().clone(),
                        database_id,
                        serialieditsync_panel,
                        cx,
                    ),
        ...
        self.pending_serialization = cx.spawn(|terminal_panel, mut cx| async move {
            ...
            let items = terminal_panel
                .update(&mut cx, |terminal_panel, cx| {
                    SerialieditsyncItems::WithSplits(serialize_pane_group(
                    ...
                    async move {
                        KEY_VALUE_STORE
                            .write_kvp(
                                ...
                                serde_json::to_string(&SerialieditsyncTerminalPanel {
                                    items,
                                    active_item_id: None,
                                    height,
                                    width,
...
impl Render for InlineAssistTabBarButton {
    fn render(&mut self, cx: &mut ViewContext<Self>) -> impl IntoElement {
        ...
        IconButton::new("terminal_inline_assistant", IconName::EditsyncAssistant)
            .icon_size(IconSize::Small)
