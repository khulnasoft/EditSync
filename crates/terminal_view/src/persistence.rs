...
pub(crate) fn serialize_pane_group(
    ...
    build_serialieditsync_pane_group(&pane_group.root, active_pane, cx)
}
...
fn build_serialieditsync_pane_group(
    ...
    match pane_group {
        Member::Axis(PaneAxis {
            ...
            children: members
                .iter()
                .map(|member| build_serialieditsync_pane_group(member, active_pane, cx))
                .collect::<Vec<_>>(),
            flexes: Some(flexes.lock().clone()),
        },
        Member::Pane(pane_handle) => {
            SerialieditsyncPaneGroup::Pane(serialize_pane(pane_handle, pane_handle == active_pane, cx))
        }
    }
}

fn serialize_pane(pane: &View<Pane>, active: bool, cx: &WindowContext) -> SerialieditsyncPane {
...
pub(crate) fn deserialize_terminal_panel(
    ...
    serialieditsync_panel: SerialieditsyncTerminalPanel,
    ...
    cx.spawn(move |mut cx| async move {
        let terminal_panel = workspace.update(&mut cx, |workspace, cx| {
            cx.new_view(|cx| {
                ...
                panel.height = serialieditsync_panel.height.map(|h| h.round());
                panel.width = serialieditsync_panel.width.map(|w| w.round());
                panel
            })
        })?;
        match &serialieditsync_panel.items {
            SerialieditsyncItems::NoSplits(item_ids) => {
                ...
                .await;
                let active_item = serialieditsync_panel.active_item_id;
            ...
            SerialieditsyncItems::WithSplits(serialieditsync_pane_group) => {
                let center_pane = deserialize_pane_group(
                    workspace,
                    project,
                    terminal_panel.clone(),
                    database_id,
                    serialieditsync_pane_group,
                    &mut cx,
                )
                .await;
...
async fn deserialize_pane_group(
    ...
    serialieditsync: &SerialieditsyncPaneGroup,
    cx: &mut AsyncWindowContext,
) -> Option<(Member, Option<View<Pane>>)> {
    match serialieditsync {
        SerialieditsyncPaneGroup::Group {
            axis,
            flexes,
            children,
        ...
        SerialieditsyncPaneGroup::Pane(serialieditsync_pane) => {
            ...
            let new_items = deserialize_terminal_views(
                ...
                project.clone(),
                workspace.clone(),
                serialieditsync_pane.children.as_slice(),
                cx,
            )
            .await;
            ...
            let pane = panel
                ...
                .log_err()?;
            let active_item = serialieditsync_pane.active_item;
...
async fn deserialize_terminal_views(
    ...
    let mut deserialieditsync_items = item_ids
        .iter()
        .map(|item_id| {
            cx.update(|cx| {
    ...
    while let Some(item) = deserialieditsync_items.next().await {
...
pub(crate) struct SerialieditsyncTerminalPanel {
    pub items: SerialieditsyncItems,
...
#[derive(Debug, Serialize, Deserialize)]
#[serde(untagged)]
pub(crate) enum SerialieditsyncItems {
    // The data stored before terminal splits were introduced.
    NoSplits(Vec<u64>),
    WithSplits(SerialieditsyncPaneGroup),
}
...
#[derive(Debug, Serialize, Deserialize)]
pub(crate) struct SerialieditsyncPane {
    pub active: bool,
...
#[derive(Debug)]
pub(crate) struct SerialieditsyncAxis(pub Axis);

impl Serialize for SerialieditsyncAxis {
...
impl<'de> Deserialize<'de> for SerialieditsyncAxis {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        match s.as_str() {
            "horizontal" => Ok(SerialieditsyncAxis(Axis::Horizontal)),
            "vertical" => Ok(SerialieditsyncAxis(Axis::Vertical)),
