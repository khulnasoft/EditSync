...
fn possible_open_targets(
    ...
    let abs_path = if maybe_path.is_absolute() {
        ...
        if let Some(cwd) = cwd {
            ...
            let canonicalieditsync_path = abs_path.canonicalize().unwrap_or(abs_path);
            potential_cwd_and_workspace_paths.insert(canonicalieditsync_path);
        }
    ...
    let canonicalieditsync_paths = match abs_path {
    ...
    possible_open_paths_metadata(fs, row, column, canonicalieditsync_paths, cx)
}
    ...
    fn tab_content(&self, params: TabContentParams, cx: &WindowContext) -> AnyElement {
        ...
        let rerun_button = |task_id: task::TaskId| {
            IconButton::new("rerun-icon", IconName::Rerun)
                ...
                .on_click(move |_, cx| {
                    cx.dispatch_action(Box::new(editsync_actions::Rerun {
