...
impl Fs for RealFs {
    ...
    #[cfg(target_os = "windows")]
    async fn trash_file(&self, path: &Path, _options: RemoveOptions) -> Result<()> {
        use util::paths::SanitieditsyncPath;
        use windows::{
            core::HSTRING,
        ...
        let path = SanitieditsyncPath::from(path.canonicalize()?);
...
fn create_temp_file(path: &Path) -> Result<NamedTempFile> {
    let temp_file = if cfg!(any(target_os = "linux", target_os = "freebsd")) {
        ...
        // See https://github.com/khulnasoft/editsync/pull/8437 for more details.
        ...
        // https://github.com/khulnasoft/editsync/issues/16571
        NamedTempFile::new_in(path.parent().unwrap_or(paths::temp_dir()))?
    } else {
        NamedTempFile::new()?
    };
