...
pub fn guess_compositor() -> &'static str {
    if std::env::var_os("EDITSYNC_HEADLESS").is_some() {
        return "Headless";
    }
...
/// Editsync's interface for handling text input from the platform's IME system
...
pub enum CursorStyle {
    ...
    /// A cursor indicating that the item/column can be resieditsync horizontally.
    /// corresponds to the CSS cursor value `col-resize`
    ResizeColumn,

    /// A cursor indicating that the item/row can be resieditsync vertically.
