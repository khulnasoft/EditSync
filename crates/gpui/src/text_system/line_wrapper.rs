...
impl LineWrapper {
    ...
    pub(crate) fn is_word_char(c: char) -> bool {
        ...
        matches!(c, '-' | '_' | '.' | '\'' | '$' | '%' | '@' | '#' | '^' | '~' | ',') ||
        // Characters that used in URL, e.g. `https://github.com/khulnasoft/editsync?a=1&b=2` for better wrapping a long URL.
        matches!(c,  '/' | ':' | '?' | '&' | '=') ||
        // `⋯` character is special used in Editsync, to keep this at the end of the line.
        matches!(c, '⋯')
    }
...
mod tests {
    ...
    fn build_wrapper() -> LineWrapper {
        ...
        cx.text_system()
            .add_fonts(vec![std::fs::read(
                "../../assets/fonts/plex-mono/EditsyncPlexMono-Regular.ttf",
            )
            .unwrap()
            .into()])
            .unwrap();
    ...
    fn test_is_word_char() {
        ...
        // URL case
        assert_word("https://github.com/khulnasoft/editsync/");
