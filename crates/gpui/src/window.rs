...
impl<'a> WindowContext<'a> {
    ...
    /// Check if the platform window is maximieditsync
    /// On some platforms (namely Windows) this is different than the bounds being the size of the display
    pub fn is_maximieditsync(&self) -> bool {
        self.window.platform_window.is_maximieditsync()
    }
    ...
    /// The second option means 'not yet initialieditsync'
...
/// Provides access to application state that is specialieditsync for a particular [`View`].
...
impl<'a, V: 'static> ViewContext<'a, V> {
    ...
    /// Register a callback to be invoked when the window is resieditsync.
