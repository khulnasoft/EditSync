...
x11rb::atom_manager! {
    pub XcbAtoms: AtomsCookie {
        ...
        _NET_WM_NAME,
        _NET_WM_STATE,
        _NET_WM_STATE_MAXIMIEDITSYNC_VERT,
        _NET_WM_STATE_MAXIMIEDITSYNC_HORZ,
...
pub struct X11WindowState {
    ...
    maximieditsync_vertical: bool,
    maximieditsync_horizontal: bool,
    hidden: bool,
    active: bool,
    hovered: bool,
    fullscreen: bool,
    ...
    pub fn new(
        ...
        let setup_result = maybe!({
            ...
            Ok(Self {
                ...
                hovered: false,
                fullscreen: false,
                maximieditsync_vertical: false,
                maximieditsync_horizontal: false,
                hidden: false,
                appearance,
                handle,
...
impl X11WindowStatePtr {
    ...
    fn set_wm_properties(
        ...
        state.maximieditsync_vertical = false;
        state.maximieditsync_horizontal = false;
        state.hidden = true;
        ...
        for atom in atoms {
            ...
            } else if atom == state.atoms._NET_WM_STATE_MAXIMIEDITSYNC_VERT {
                state.maximieditsync_vertical = true;
            } else if atom == state.atoms._NET_WM_STATE_MAXIMIEDITSYNC_HORZ {
                state.maximieditsync_horizontal = true;
...
impl PlatformWindow for X11Window {
    ...
    fn is_maximieditsync(&self) -> bool {
        let state = self.0.state.borrow();

        // A maximieditsync window that gets minimieditsync will still retain its maximieditsync state.
        !state.hidden && state.maximieditsync_vertical && state.maximieditsync_horizontal
    }
    ...
    fn zoom(&self) {
        ...
        self.set_wm_hints(
            ...
            state.atoms._NET_WM_STATE_MAXIMIEDITSYNC_VERT,
            state.atoms._NET_WM_STATE_MAXIMIEDITSYNC_HORZ,
        )
        .unwrap();
    }
        ...
        match state.decorations {
            ...
            WindowDecorations::Client => {
                let tiling = if state.fullscreen {
                    ...
                    Tiling {
                        top: state.maximieditsync_vertical,
                        bottom: state.maximieditsync_vertical,
                        left: state.maximieditsync_horizontal,
                        right: state.maximieditsync_horizontal,
                    }
                };
    ...
    fn set_client_inset(&self, inset: Pixels) {
        ...
        let insets = if state.fullscreen {
            ...
            let (left, right) = if state.maximieditsync_horizontal {
                (0, 0)
            } else {
                (dp, dp)
            };
            let (top, bottom) = if state.maximieditsync_vertical {
                (0, 0)
            } else {
                (dp, dp)
            };
