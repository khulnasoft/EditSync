...
struct TestDispatcherState {
    ...
    deprioritieditsync_background: Vec<Runnable>,
    ...
    deprioritieditsync_task_labels: HashSet<TaskLabel>,
...
impl TestDispatcher {
    pub fn new(random: StdRng) -> Self {
        ...
        let state = TestDispatcherState {
            ...
            background: Vec::new(),
            deprioritieditsync_background: Vec::new(),
            delayed: Vec::new(),
            time: Duration::ZERO,
            ...
            deprioritieditsync_task_labels: Default::default(),
    ...
    pub fn tick(&self, background_only: bool) -> bool {
        ...
        if foreground_len == 0 && background_len == 0 {
            let deprioritieditsync_background_len = state.deprioritieditsync_background.len();
            if deprioritieditsync_background_len == 0 {
                return false;
            }
            let ix = state.random.gen_range(0..deprioritieditsync_background_len);
            main_thread = false;
            runnable = state.deprioritieditsync_background.swap_remove(ix);
        } else {
    ...
    pub fn deprioritize(&self, task_label: TaskLabel) {
        self.state
            .lock()
            .deprioritieditsync_task_labels
            .insert(task_label);
    }
    ...
    fn dispatch(&self, runnable: Runnable, label: Option<TaskLabel>) {
        {
            ...
            if label.map_or(false, |label| {
                state.deprioritieditsync_task_labels.contains(&label)
            }) {
                state.deprioritieditsync_background.push(runnable);
            } else {
                state.background.push(runnable);
            }
        }
