...
fn handle_size_msg(
    ...
    // Don't resize the renderer when the window is minimieditsync, but record that it was minimieditsync so
    // that on restore the swap chain can be recreated via `update_drawable_size_even_if_unchanged`.
    if wparam.0 == SIZE_MINIMIEDITSYNC as usize {
        lock.restore_from_minimieditsync = lock.callbacks.request_frame.take();
        return Some(0);
    }
    ...
    if lock.restore_from_minimieditsync.is_some() {
        lock.renderer
            .update_drawable_size_even_if_unchanged(new_size);
        lock.callbacks.request_frame = lock.restore_from_minimieditsync.take();
    } else {
...
fn handle_calc_client_size(
    ...
    let is_maximieditsync = state_ptr.state.borrow().is_maximieditsync();
    let insets = get_client_area_insets(handle, is_maximieditsync, state_ptr.windows_version);
...
fn get_client_area_insets(
    handle: HWND,
    is_maximieditsync: bool,
    windows_version: WindowsVersion,
) -> RECT {
    // For maximieditsync windows, Windows outdents the window rect from the screen's client rect
    ...
    // monitors.
    //
    // For non-maximieditsync windows, things become complicated:
    //
    // - On Windows 10
    // The top inset must be zero, since if there is any nonclient area, Windows will draw
    // a full native titlebar outside the client area. (This doesn't occur in the maximieditsync
    // case.)
    //
    // - On Windows 11
    ...
    let top_insets = if is_maximieditsync {
        frame_thickness
    } else {
