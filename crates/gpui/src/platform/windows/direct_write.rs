...
fn get_font_names_from_collection(
    ...
    unsafe {
        ...
        for index in 0..family_count {
            ...
            let Some(localieditsync_family_name) = font_family.GetFamilyNames().log_err() else {
                continue;
            };
            let Some(family_name) = get_name(localieditsync_family_name, locale).log_err() else {
                continue;
            };
...
fn get_font_identifier_and_font_struct(
    ...
    let localieditsync_family_name = unsafe { font_face.GetFamilyNames().log_err() }?;
    let family_name = get_name(localieditsync_family_name, locale).log_err()?;
...
#[inline]
fn get_name(string: IDWriteLocalieditsyncStrings, locale: &str) -> Result<String> {
