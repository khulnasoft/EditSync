...
impl Bounds<Pixels> {
    ...
    /// Generate maximieditsync bounds for the given display or primary display if none is provided
    pub fn maximieditsync(display_id: Option<DisplayId>, cx: &AppContext) -> Self {
        let display = display_id
            .and_then(|id| cx.find_display(id))
            .or_else(|| cx.primary_display());
    ...
    T: Add<T, Output = T> + Clone + Default + Debug,
{
    ...
    ///
    /// ```
    /// # use editsync::{Bounds, Corner, Point, Size};
    /// let bounds = Bounds {
    ///     origin: Point { x: 0, y: 0 },
    ///     size: Size { width: 10, height: 20 },
    /// };
...
impl<T: Clone + Default + Debug> Edges<T> {
    ...
    ///
    /// This function creates an `Edges` instance with the `top`, `right`, `bottom`, and `left` fields all initialieditsync
    ...
    T: Clone + Default + Debug,
{
    ...
    ///
    /// This function creates a `Corners` instance with the `top_left`, `top_right`, `bottom_right`, and `bottom_left` fields all initialieditsync
