...
/// (Though mostly when developing Editsync itself, you just need to add a new line to
...
/// bubbles up from the bottom. For example in Editsync, the Workspace is the top-level view, which contains Pane's, which contain Editors. If there are conflicting keybindings defined
