...
GPUI is still in active development as we work on the Editsync code editor and isn't yet on crates.io. You'll also need to use the latest version of stable Rust and be on macOS or Linux. Add the following to your `Cargo.toml`:

```toml
gpui = { git = "https://github.com/khulnasoft/editsync" }
```
...
Currently, the best way to learn about these APIs is to read the Editsync source code, ask us about it at a fireside hack, or drop a question in the [Editsync Discord](https://editsync.khulnasoft.com/community-links). We're working on improving the documentation, creating more examples, and will be publishing more guides to GPUI on our [blog](https://editsync.khulnasoft.com/blog).
