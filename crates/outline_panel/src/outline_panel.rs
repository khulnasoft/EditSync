...
struct SerialieditsyncOutlinePanel {
    width: Option<Pixels>,
    active: Option<bool>,
}
...
impl OutlinePanel {
    pub async fn load(
        ...
        let serialieditsync_panel = cx
            ...
            .log_err()
            .flatten()
            .map(|panel| serde_json::from_str::<SerialieditsyncOutlinePanel>(&panel))
            .transpose()
            .log_err()
            .flatten();
        ...
        workspace.update(&mut cx, |workspace, cx| {
            ...
            if let Some(serialieditsync_panel) = serialieditsync_panel {
                panel.update(cx, |panel, cx| {
                    ...
                    panel.active = serialieditsync_panel.active.unwrap_or(false);
                    cx.notify();
                });
            }
            panel
            ...
            async move {
                KEY_VALUE_STORE
                    .write_kvp(
                        ...
                        serde_json::to_string(&SerialieditsyncOutlinePanel { width, active })?,
                    )
                    .await?;
                anyhow::Ok(())
            }
