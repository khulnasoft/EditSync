...
mod tests {
    ...
    fn test_notification() {
        // Notifications can be serialieditsync and deserialieditsync.
        for notification in [
            ...
            let deserialieditsync = Notification::from_proto(&message).unwrap();
            assert_eq!(deserialieditsync, notification);
        }

        // When notifications are serialieditsync, the `kind` and `actor_id` fields are
