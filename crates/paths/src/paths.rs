//! Paths to locations used by Editsync.
...
/// Returns the relative path to the editsync_server directory on the ssh host.
pub fn remote_server_dir_relative() -> &'static Path {
    Path::new(".editsync_server")
}

/// Returns the path to the configuration directory used by Editsync.
pub fn config_dir() -> &'static PathBuf {
    ...
    CONFIG_DIR.get_or_init(|| {
        ...
        home_dir().join(".config").join("editsync")
    })
}

/// Returns the path to the support directory used by Editsync.
pub fn support_dir() -> &'static PathBuf {
    static SUPPORT_DIR: OnceLock<PathBuf> = OnceLock::new();
    SUPPORT_DIR.get_or_init(|| {
        if cfg!(target_os = "macos") {
            return home_dir().join("Library/Application Support/Editsync");
        }
...
/// Returns the path to the temp directory used by Editsync.
pub fn temp_dir() -> &'static PathBuf {
    ...
    TEMP_DIR.get_or_init(|| {
        ...
        home_dir().join(".cache").join("editsync")
    })
}
...
pub fn logs_dir() -> &'static PathBuf {
    ...
    LOGS_DIR.get_or_init(|| {
        if cfg!(target_os = "macos") {
            home_dir().join("Library/Logs/Editsync")
        } else {
...
/// Returns the path to the Editsync server directory on this SSH host.
...
/// Returns the path to the `Editsync.log` file.
pub fn log_file() -> &'static PathBuf {
    static LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    LOG_FILE.get_or_init(|| logs_dir().join("Editsync.log"))
}

/// Returns the path to the `Editsync.log.old` file.
pub fn old_log_file() -> &'static PathBuf {
    static OLD_LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    OLD_LOG_FILE.get_or_init(|| logs_dir().join("Editsync.log.old"))
}
...
/// # Arguments
///
/// * `dev_mode` - If true, assumes the current working directory is the Editsync repository.
...
///
/// This is where language servers are downloaded to for languages built-in to Editsync.
...
/// Returns the relative path to a `.editsync` folder within a project.
