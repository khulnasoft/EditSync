...
/// The current extension [`SchemaVersion`] supported by Editsync.
const CURRENT_SCHEMA_VERSION: SchemaVersion = SchemaVersion(1);

/// Returns the [`SchemaVersion`] range that is compatible with this version of Editsync.
pub fn schema_version_range() -> RangeInclusive<SchemaVersion> {
    SchemaVersion::ZERO..=CURRENT_SCHEMA_VERSION
}

/// Returns whether the given extension version is compatible with this version of Editsync.
...
actions!(editsync, [ReloadExtensions]);
...
impl ExtensionStore {
    ...
    /// Installs any extensions that should be included with Editsync by default.
    ///
    ...
    fn fetch_extensions_from_api(
        ...
        let url = self.http_client.build_editsync_api_url(path, query);
