...
pub struct WasmExtension {
    ...
    #[allow(unused)]
    pub editsync_api_version: SemanticVersion,
}
...
impl WasmHost {
    ...
    pub fn load_extension(
        ...
        executor.clone().spawn(async move {
            let editsync_api_version = parse_wasm_extension_version(&manifest.id, &wasm_bytes)?;
...
pub fn parse_wasm_extension_version(
    ...
    for part in wasmparser::Parser::new(0).parse_all(wasm_bytes) {
        ...
        {
            if s.name() == "editsync:api-version" {
                version = parse_wasm_extension_version_custom_section(s.data());
                if version.is_none() {
                    bail!(
                        "extension {} has invalid editsync:api-version section: {:?}",
                        extension_id,
                        s.data()
                    );
                }
