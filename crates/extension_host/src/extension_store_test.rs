...
async fn test_extension_store(cx: &mut TestAppContext) {
    ...
    let mut expected_index = ExtensionIndex {
        extensions: [
            (
                ...
                ExtensionIndexEntry {
                    manifest: Arc::new(ExtensionManifest {
                        id: "editsync-ruby".into(),
                        name: "Editsync Ruby".into(),
                        version: "1.0.0".into(),
            ...
            (
                "editsync-monokai".into(),
                ExtensionIndexEntry {
                    manifest: Arc::new(ExtensionManifest {
                        id: "editsync-monokai".into(),
                        name: "Editsync Monokai".into(),
                        version: "2.0.0".into(),
                        schema_version: SchemaVersion::ZERO,
                        description: None,
                        authors: vec![],
        ...
        themes: [
            (
                ...
                ExtensionIndexThemeEntry {
                    extension: "editsync-monokai".into(),
    ...
    fs.insert_tree(
        "/the-extension-dir/installed/editsync-gruvbox",
        json!({
            "extension.json": r#"{
                "id": "editsync-gruvbox",
                "name": "Editsync Gruvbox",
                "version": "1.0.0",
                "themes": {
    ...
    expected_index.extensions.insert(
        "editsync-gruvbox".into(),
        ExtensionIndexEntry {
            manifest: Arc::new(ExtensionManifest {
                id: "editsync-gruvbox".into(),
                name: "Editsync Gruvbox".into(),
                version: "1.0.0".into(),
                schema_version: SchemaVersion::ZERO,
                description: None,
                authors: vec![],
    ...
    expected_index.themes.insert(
        "Gruvbox".into(),
        ExtensionIndexThemeEntry {
            extension: "editsync-gruvbox".into(),
    ...
    // Create new extension store, as if Editsync were restarting.
    drop(store);
    let store = cx.new_model(|cx| {
        ExtensionStore::new(
            PathBuf::from("/the-extension-dir"),
    ...
    store.update(cx, |store, cx| {
        store.uninstall_extension("editsync-ruby".into(), cx)
    });

    cx.executor().advance_clock(RELOAD_DEBOUNCE_DURATION);
    expected_index.extensions.remove("editsync-ruby");
