...
impl LspAdapter for RustLspAdapter {
    ...
    fn process_diagnostics(&self, params: &mut lsp::PublishDiagnosticsParams) {
        ...
        for diagnostic in &mut params.diagnostics {
            ...
            {
                if let Cow::Owned(sanitieditsync) = REGEX.replace_all(message, "`$1`") {
                    *message = sanitieditsync;
                }
            }
        }
    }
    ...
    fn associated_tasks(
        ...
        Some(TaskTemplates(vec![
            TaskTemplate {
                ...
                cwd: Some("$EDITSYNC_DIRNAME".to_owned()),
...
// Output example in the root of Editsync project:
// ```sh
// ❯ cargo pkgid editsync
// path+file:///absolute/path/to/project/editsync/crates/editsync#0.131.0
// ```
...
mod tests {
    ...
    fn test_retrieve_package_id_and_bin_name_from_metadata() {
        for (input, absolute_path, expected) in [
            (
                r#"{"packages":[{"id":"path+file:///path/to/editsync/crates/editsync#0.131.0","targets":[{"name":"editsync","kind":["bin"],"src_path":"/path/to/editsync/src/main.rs"}]}]}"#,
                "/path/to/editsync/src/main.rs",
                Some((
                    "path+file:///path/to/editsync/crates/editsync#0.131.0",
                    "editsync",
                    TargetKind::Bin,
                )),
            ),
