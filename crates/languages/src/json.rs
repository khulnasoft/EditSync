...
pub(super) fn json_task_context() -> ContextProviderWithTasks {
    ContextProviderWithTasks::new(TaskTemplates(vec![
        TaskTemplate {
            label: "package script $EDITSYNC_CUSTOM_script".to_owned(),
            command: "npm --prefix $EDITSYNC_DIRNAME run".to_owned(),
            args: vec![VariableName::Custom("script".into()).template_value()],
            tags: vec!["package-script".into()],
            ..TaskTemplate::default()
        },
        TaskTemplate {
            label: "composer script $EDITSYNC_CUSTOM_script".to_owned(),
            command: "composer -d $EDITSYNC_DIRNAME".to_owned(),
            args: vec![VariableName::Custom("script".into()).template_value()],
...
impl LspAdapter for NodeVersionAdapter {
    ...
    async fn fetch_latest_server_version(
        ...
        let release = latest_github_release(
            "khulnasoft/package-version-server",
            true,
            false,
