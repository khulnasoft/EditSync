...
struct FamilyMetadata {
    ...
    ///
    /// Use this to ensure certain Editsync syntax tokens are matched
...
enum Command {
    ...
    PrintSchema,
    /// Converts a VSCode theme to Editsync format [default]
    Convert {
...
fn convert(theme_file_path: PathBuf, output: Option<PathBuf>, warn_on_missing: bool) -> Result<()> {
    ...
    theme.as_object_mut().unwrap().insert(
        "$schema".to_string(),
        serde_json::Value::String(EDITSYNC_THEME_SCHEMA_URL.to_string()),
    );
