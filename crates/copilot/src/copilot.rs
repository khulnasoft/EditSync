...
impl CopilotServer {
    fn as_authenticated(&mut self) -> Result<&mut RunningCopilotServer> {
        ...
        if matches!(server.sign_in_status, SignInStatus::Authorieditsync { .. }) {
            Ok(server)
        } else {
...
impl Status {
    pub fn is_authorieditsync(&self) -> bool {
        matches!(self, Status::Authorieditsync)
    }
...
impl Copilot {
    ...
    pub fn fake(cx: &mut gpui::TestAppContext) -> (Model<Self>, lsp::FakeLanguageServer) {
        ...
        let this = cx.new_model(|cx| Self {
            ...
            server: CopilotServer::Running(RunningCopilotServer {
                lsp: Arc::new(server),
                sign_in_status: SignInStatus::Authorieditsync,
    ...
    pub fn sign_in(&mut self, cx: &mut ModelContext<Self>) -> Task<Result<()>> {
        if let CopilotServer::Running(server) = &mut self.server {
            let task = match &server.sign_in_status {
                SignInStatus::Authorieditsync { .. } => Task::ready(Ok(())).shared(),
                SignInStatus::SigningIn { task, .. } => {
                    cx.notify();
                    task.clone()
                }
                SignInStatus::SignedOut | SignInStatus::Unauthorieditsync { .. } => {
    ...
    pub fn register_buffer(&mut self, buffer: &Model<Buffer>, cx: &mut ModelContext<Self>) {
        ...
        {
            if !matches!(status, SignInStatus::Authorieditsync { .. }) {
                return;
            }
        ...
        match &self.server {
            ...
            CopilotServer::Running(RunningCopilotServer { sign_in_status, .. }) => {
                match sign_in_status {
                    SignInStatus::Authorieditsync { .. } => Status::Authorieditsync,
                    SignInStatus::Unauthorieditsync { .. } => Status::Unauthorieditsync,
        ...
        if let Ok(server) = self.server.as_running() {
            match lsp_status {
                ...
                | request::SignInStatus::AlreadySignedIn { .. } => {
                    server.sign_in_status = SignInStatus::Authorieditsync;
                ...
                request::SignInStatus::NotAuthorieditsync { .. } => {
                    server.sign_in_status = SignInStatus::Unauthorieditsync;
                    for buffer in self.buffers.iter().cloned().collect::<Vec<_>>() {
