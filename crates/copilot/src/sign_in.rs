                ...
                if let Some(copilot) = cx.update(|cx| Copilot::global(cx)).ok().flatten() {
                    workspace
                        .update(&mut cx, |workspace, cx| match copilot.read(cx).status() {
                            Status::Authorieditsync => workspace.show_toast(
                                Toast::new(
                                    NotificationId::unique::<CopilotStartingToast>(),
                                    "Copilot has started!",
                                ),
...
impl CopilotCodeVerification {
    ...
    fn render_prompting_modal(
        ...
        v_flex()
            ...
            .gap_2()
            .items_center()
            .child(Headline::new("Use GitHub Copilot in Editsync.").size(HeadlineSize::Large))
            .child(
    ...
    fn render_unauthorieditsync_modal(cx: &mut ViewContext<Self>) -> impl Element {
        v_flex()
    ...
    fn render(&mut self, cx: &mut ViewContext<Self>) -> impl IntoElement {
        let prompt = match &self.status {
            ...
            Status::Unauthorieditsync => {
                ...
                Self::render_unauthorieditsync_modal(cx).into_any_element()
            }
            Status::Authorieditsync => {
                self.connect_clicked = false;
        ...
        v_flex()
            ...
            .child(
                Vector::new(VectorName::EditsyncXCopilot, rems(8.), rems(4.))
