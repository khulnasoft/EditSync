...
fn test_select_language(cx: &mut AppContext) {
    ...
    assert_eq!(
        registry
            .language_for_file(&file("editsync/cars"), None, cx)
            .map(|l| l.name()),
        None
    );
    assert_eq!(
        registry
            .language_for_file(&file("editsync/a.cars"), None, cx)
            .map(|l| l.name()),
        None
    );
    assert_eq!(
        registry
            .language_for_file(&file("editsync/sumk"), None, cx)
            .map(|l| l.name()),
        None
    );
}
...
fn test_language_at_with_hidden_languages(cx: &mut AppContext) {
    ...
    cx.new_model(|cx| {
        let text = r#"
            this is an *emphasieditsync* word.
        "#
        .unindent();
