...
pub trait File: Send + Sync {
    ...
    /// Return whether Editsync considers this to be a private file.
...
pub enum DiskState {
    /// File created in Editsync that has not been saved.
    New,
...
impl Buffer {
    ...
    pub fn local_normalieditsync(
        base_text_normalieditsync: Rope,
        line_ending: LineEnding,
        cx: &ModelContext<Self>,
    ) -> Self {
        Self::build(
            TextBuffer::new_normalieditsync(
                0,
                cx.entity_id().as_non_zero_u64().into(),
                line_ending,
                base_text_normalieditsync,
            ),
            None,
            Capability::ReadWrite,
        )
