...
trait InstalledApp {
    fn editsync_version_string(&self) -> String;
...
#[command(
    ...
    after_help = "To read from stdin, append '-' (e.g. 'ps axf | editsync -')"
)]
struct Args {
    ...
    /// Print Editsync's version and the app path.
    #[arg(short, long)]
    version: bool,
    /// Run editsync in the foreground (useful for debugging)
    #[arg(long)]
    foreground: bool,
    /// Custom path to Editsync.app or the editsync binary
    #[arg(long)]
    editsync: Option<PathBuf>,
    /// Run editsync in dev-server mode
    #[arg(long)]
    dev_server_token: Option<String>,
    /// Uninstall Editsync from user system
    #[cfg(all(
...
fn parse_path_with_position(argument_str: &str) -> anyhow::Result<String> {
    let canonicalieditsync = match Path::new(argument_str).canonicalize() {
    ...
    Ok(canonicalieditsync.to_string(|path| path.to_string_lossy().to_string()))
}
...
fn main() -> Result<()> {
    ...
    let app = Detect::detect(args.editsync.as_deref()).context("Bundle detection")?;

    if args.version {
        println!("{}", app.editsync_version_string());
        return Ok(());
    }
    ...
    if args.uninstall {
        ...
        let status = std::process::Command::new("sh")
            .arg(&script_path)
            .env("EDITSYNC_CHANNEL", &*release_channel::RELEASE_CHANNEL_NAME)
            .status()
    ...
    let (server, server_name) =
        IpcOneShotServer::<IpcHandshake>::new().context("Handshake before Editsync spawn")?;
    let url = format!("editsync-cli://{server_name}");
    ...
    // On Linux, desktop entry uses `cli` to spawn `editsync`, so we need to load env vars from the shell
    ...
    if let Some(_) = args.dev_server_token {
        return Err(anyhow::anyhow!(
            "Dev servers were removed in v0.157.x please upgrade to SSH remoting: https://editsync.khulnasoft.com/docs/remote-development"
        ))?;
    }
    ...
    let sender: JoinHandle<anyhow::Result<()>> = thread::spawn({
        ...
        move || {
            let (_, handshake) = server.accept().context("Handshake after Editsync spawn")?;
...
mod flatpak {
    ...
    const EXTRA_LIB_ENV_NAME: &'static str = "EDITSYNC_FLATPAK_LIB_PATH";
    const NO_ESCAPE_ENV_NAME: &'static str = "EDITSYNC_FLATPAK_NO_ESCAPE";
    ...
    pub fn try_restart_to_host() {
        if let Some(flatpak_dir) = get_flatpak_dir() {
            ...
            args.push(flatpak_dir.join("bin").join("editsync").into());

            let mut is_app_location_set = false;
            for arg in &env::args_os().collect::<Vec<_>>()[1..] {
                args.push(arg.clone());
                is_app_location_set |= arg == "--editsync";
            }

            if !is_app_location_set {
                args.push("--editsync".into());
                args.push(flatpak_dir.join("libexec").join("editsync-editor").into());
            }
    ...
    pub fn set_bin_if_no_escape(mut args: super::Args) -> super::Args {
        if env::var(NO_ESCAPE_ENV_NAME).is_ok()
            && env::var("FLATPAK_ID").map_or(false, |id| id.starts_with("dev.editsync.Editsync"))
        {
    ...
    fn get_flatpak_dir() -> Option<PathBuf> {
        ...
        if let Ok(flatpak_id) = env::var("FLATPAK_ID") {
            if !flatpak_id.starts_with("dev.editsync.Editsync") {
                return None;
            }
...
mod mac_os {
    ...
    impl InstalledApp for Bundle {
            ...
            match self {
                Self::App { app_bundle, .. } => {
                    ...
                    let status = unsafe {
                        ...
                        // equivalent to: open editsync-cli:... -a /Applications/Editsync\ Preview.app
                        let urls_to_open =
                ...
                Self::LocalPath { executable, .. } => {
                    ...
                    let subprocess_stdout_file = fs::File::create(
                        executable_parent.join("editsync_dev.log"),
                    )
        ...
        fn run_foreground(&self, ipc_url: String) -> io::Result<ExitStatus> {
            let path = match self {
                Bundle::App { app_bundle, .. } => app_bundle.join("Contents/MacOS/editsync"),
