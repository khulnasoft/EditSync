...
mod tests {
    ...
    async fn test_newlines_dont_new_paragraphs() {
        let parsed = parse("Some text **that is bolded**\n and *italicieditsync*").await;

        assert_eq!(
            parsed.children,
            vec![p("Some text that is bolded and italicieditsync", 0..46)]
        );
    }

    #[gpui::test]
    async fn test_heading_with_paragraph() {
        let parsed = parse("# Editsync\nThe editor").await;

        assert_eq!(
            parsed.children,
            vec![h1(text("Editsync", 2..5), 0..6), p("The editor", 6..16),]
        );
    }
    ...
    async fn test_bold_italic_text() {
        let parsed = parse("Some text **that is bolded** and *italicieditsync*").await;

        assert_eq!(
            parsed.children,
            vec![p("Some text that is bolded and italicieditsync", 0..45)]
        );
    }
    ...
    async fn test_raw_links_detection() {
        let parsed = parse("Checkout this https://editsync.khulnasoft.com link").await;
    ...
    async fn test_image_links_detection() {
        let parsed = parse("![test](https://blog.logrocket.com/wp-content/uploads/2024/04/exploring-editsync-open-source-code-editor-rust-2.png)").await;
        ...
        assert_eq!(
            ...
            MarkdownParagraphChunk::Image(Image {
                source_range: 0..111,
                link: Link::Web {
                    url: "https://blog.logrocket.com/wp-content/uploads/2024/04/exploring-editsync-open-source-code-editor-rust-2.png".to_string(),
                },
