...
#[gpui::test]
async fn test_channel_requires_editsync_cla(cx_a: &mut TestAppContext, cx_b: &mut TestAppContext) {
    let mut server = TestServer::start(cx_a.executor()).await;
    ...
    // Create a parent channel that requires the Editsync CLA
    ...
    server
        .app_state
        .db
        .set_channel_requires_editsync_cla(ChannelId::from_proto(parent_channel_id.0), true)
        .await
        .unwrap();
