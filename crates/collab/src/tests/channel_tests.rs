...
async fn test_channel_room(
    ...
    cx_a.read(|cx| {
        client_a.channel_store().read_with(cx, |channels, _| {
            assert_participants_eq(
                channels.channel_participants(editsync_id),
                &[client_a.user_id().unwrap()],
            );
        })
    });
    ...
    cx_a.read(|cx| {
        client_a.channel_store().read_with(cx, |channels, _| {
            assert_participants_eq(channels.channel_participants(editsync_id), &[]);
        })
    });
...
async fn test_channel_jumping(executor: BackgroundExecutor, cx_a: &mut TestAppContext) {
    ...
    let editsync_id = server
        .make_channel("editsync", None, (&client_a, cx_a), &mut [])
        .await;
    let rust_id = server
...
async fn test_channel_link_notifications(
    ...
    let channels = server
        .make_channel_tree(&[("editsync", None)], (&client_a, cx_a))
        .await;
    let editsync_channel = channels[0];
    ...
    client_b
        .channel_store()
        .update(cx_b, |channel_store, cx| {
            channel_store.respond_to_channel_invite(editsync_channel, true, cx)
        })
        .await
        .unwrap();
    ...
    // we have an admin (a), member (b) and guest (c) all part of the editsync channel.
    ...
    let active_channel = client_a
        .channel_store()
        .update(cx_a, |channel_store, cx| {
            channel_store.create_channel("active", Some(editsync_channel), cx)
        })
        .await
        .unwrap();
    ...
    assert_channels_list_shape(
        client_a.channel_store(),
        cx_a,
        &[(editsync_channel, 0), (active_channel, 1)],
    );
    assert_channels_list_shape(
        client_b.channel_store(),
        cx_b,
    ...
    assert_channels_list_shape(client_c.channel_store(), cx_c, &[(editsync_channel, 0)]);

    let vim_channel = client_a
        .channel_store()
        .update(cx_a, |channel_store, cx| {
            channel_store.create_channel("vim", Some(editsync_channel), cx)
        })
        .await
        .unwrap();
    ...
    assert_channels_list_shape(
        client_a.channel_store(),
        cx_a,
        &[(editsync_channel, 0), (active_channel, 1), (vim_channel, 1)],
    );
    assert_channels_list_shape(
        client_b.channel_store(),
        cx_b,
    ...
    assert_channels_list_shape(
        ...
        cx_c,
        &[(editsync_channel, 0), (vim_channel, 1)],
    );

    let helix_channel = client_a
        .channel_store()
        .update(cx_a, |channel_store, cx| {
            channel_store.create_channel("helix", Some(editsync_channel), cx)
        })
        .await
        .unwrap();
    ...
    assert_channels_list_shape(
        client_c.channel_store(),
        cx_c,
        &[(editsync_channel, 0), (vim_channel, 1), (helix_channel, 2)],
    );
}
...
async fn test_channel_membership_notifications(
    ...
    // we have an admin (a), and a guest (b) with access to all of editsync, and membership in vim.
    assert_channels(
        client_b.channel_store(),
        cx_b,
        &[
    ...
    client_b.channel_store().update(cx_b, |channel_store, _| {
        channel_store.is_channel_admin(editsync_channel)
    });
