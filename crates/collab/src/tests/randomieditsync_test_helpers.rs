...
struct TestPlan<T: RandomieditsyncTest> {
    rng: StdRng,
    replay: bool,
...
pub async fn run_randomieditsync_test<T: RandomieditsyncTest>(
    cx: &mut TestAppContext,
...
pub fn save_randomieditsync_test_plan() {
...
impl<T: RandomieditsyncTest> TestPlan<T> {
    ...
    async fn apply_server_operation(
        ...
        match operation {
            ...
            ServerOperation::RestartServer => {
                ...
                let environment = &server.app_state.config.editsync_environment;
