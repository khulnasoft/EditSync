...
const CRASH_REPORTS_BUCKET: &str = "editsync-crash-reports";
...
pub struct EditsyncChecksumHeader(Vec<u8>);

impl Header for EditsyncChecksumHeader {
    fn name() -> &'static HeaderName {
        static EDITSYNC_CHECKSUM_HEADER: OnceLock<HeaderName> = OnceLock::new();
        EDITSYNC_CHECKSUM_HEADER.get_or_init(|| HeaderName::from_static("x-editsync-checksum"))
    }
...
pub async fn post_crash(
    ...
    if bundle_id == "dev.editsync.Editsync-Dev" {
    ...
    let recent_panic_on: Option<i64> = headers
        .get("x-editsync-panicked-on")
        .and_then(|h| h.to_str().ok())
        .and_then(|s| s.parse().ok());

    let installation_id = headers
        .get("x-editsync-installation-id")
        .and_then(|h| h.to_str().ok())
        .map(|s| s.to_string())
        .unwrap_or_default();
...
pub async fn post_hang(
    Extension(app): Extension<Arc<AppState>>,
    TypedHeader(EditsyncChecksumHeader(checksum)): TypedHeader<EditsyncChecksumHeader>,
    body: Bytes,
) -> Result<()> {
...
pub fn calculate_json_checksum(app: Arc<AppState>, json: &impl AsRef<[u8]>) -> Option<Vec<u8>> {
    let checksum_seed = app.config.editsync_client_checksum_seed.as_ref()?;
...
fn for_snowflake(
    ...
    body.events.into_iter().flat_map(move |event| {
        ...
        // dictionary. See https://app.amplitude.com/data/editsync/Editsync/sources/detail/production/falcon%3A159998
