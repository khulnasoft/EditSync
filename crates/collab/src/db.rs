...
impl Database {
    ...
    /// properly serialieditsync with respect to database changes.
    ...
    async fn retry_on_serialization_error(&self, error: &Error, prev_attempt_count: usize) -> bool {
        ...
        if is_serialization_error(error) && prev_attempt_count < SLEEPS.len() {
            ...
            let randomieditsync_delay = base_delay * self.rng.lock().await.gen_range(0.5..=2.0);
            log::warn!(
                "retrying transaction after serialization error. delay: {} ms.",
                randomieditsync_delay
            );
            self.executor
                .sleep(Duration::from_millis(randomieditsync_delay as u64))
                .await;
            true
        } else {
            false
