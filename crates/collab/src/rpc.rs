...
pub use connection_pool::{ConnectionPool, EditsyncVersion};
...
impl Server {
        ...
        self.app_state.executor.spawn_detached(
            async move {
                ...
                if let Some((room_ids, channel_ids)) = app_state
                    .db
                    .stale_server_resource_ids(&app_state.config.editsync_environment, server_id)
                    .await
                    .trace_err()
                {
                ...
                app_state
                    .db
                    .delete_stale_servers(&app_state.config.editsync_environment, server_id)
                    .await
                    .trace_err();
            }
            .instrument(span),
    ...
    pub fn handle_connection(
        ...
        address: String,
        principal: Principal,
        editsync_version: EditsyncVersion,
        geoip_country_code: Option<String>,
        system_id: Option<String>,
        ...
        async move {
            ...
            let user_agent = format!("Editsync Server/{}", env!("CARGO_PKG_VERSION"));
            ...
            if let Err(error) = this.send_initial_client_update(connection_id, &principal, editsync_version, send_connection_id, &session).await {
        ...
        match principal {
            Principal::User(user) | Principal::Impersonated { user, admin: _ } => {
...

                {
                    ...
                    pool.add_connection(connection_id, user.id, user.admin, editsync_version);
                    self.peer.send(
                        connection_id,
                ...
                if should_auto_subscribe_to_channels(editsync_version) {
...
impl Header for AppVersionHeader {
    fn name() -> &'static HeaderName {
        static EDITSYNC_APP_VERSION: OnceLock<HeaderName> = OnceLock::new();
        EDITSYNC_APP_VERSION.get_or_init(|| HeaderName::from_static("x-editsync-app-version"))
    }
...
pub async fn handle_websocket_request(
    ...
    let Some(version) = app_version_header.map(|header| EditsyncVersion(header.0 .0)) else {
        return (
            StatusCode::UPGRADE_REQUIRED,
...
fn should_auto_subscribe_to_channels(version: EditsyncVersion) -> bool {
    version.0.minor() < 139
}
...
async fn join_channel_internal(
    ...
    let joined_room = {
        ...
        // If editsync quits without leaving the room, and the user re-opens editsync before the
...
async fn count_language_model_tokens(
    ...
    let rate_limit: Box<dyn RateLimit> = match session.current_plan(&session.db().await).await? {
        proto::Plan::EditsyncPro => Box::new(EditsyncProCountLanguageModelTokensRateLimit),
...
struct EditsyncProCountLanguageModelTokensRateLimit;

impl RateLimit for EditsyncProCountLanguageModelTokensRateLimit {
    ...
    fn db_name(&self) -> &'static str {
        "editsync-pro:count-language-model-tokens"
    }
}
...
struct EditsyncProComputeEmbeddingsRateLimit;

impl RateLimit for EditsyncProComputeEmbeddingsRateLimit {
    fn capacity(&self) -> usize {
    ...
    fn db_name(&self) -> &'static str {
        "editsync-pro:compute-embeddings"
    }
}
...
async fn compute_embeddings(
    ...
    let rate_limit: Box<dyn RateLimit> = match session.current_plan(&session.db().await).await? {
        proto::Plan::EditsyncPro => Box::new(EditsyncProComputeEmbeddingsRateLimit),
