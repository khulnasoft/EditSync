...
impl Database {
    ...
    pub async fn create_channel(
        ...
        self.transaction(move |tx| async move {
            ...
            let channel = channel::ActiveModel {
                ...
                requires_editsync_cla: ActiveValue::NotSet,
            }
            .insert(&*tx)
            .await?;
    ...
    #[cfg(test)]
    pub async fn set_channel_requires_editsync_cla(
        &self,
        channel_id: ChannelId,
        requires_editsync_cla: bool,
    ) -> Result<()> {
        self.transaction(move |tx| async move {
            let channel = self.get_channel_internal(channel_id, &tx).await?;
            let mut model = channel.into_active_model();
            model.requires_editsync_cla = ActiveValue::Set(requires_editsync_cla);
