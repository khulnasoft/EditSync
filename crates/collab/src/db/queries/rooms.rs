...
impl Database {
    ...
    async fn check_user_has_signed_cla(
        ...
        if let Some(channel) = channel {
            let requires_editsync_cla = channel.requires_editsync_cla
                || channel::Entity::find()
                    .filter(
                        channel::Column::Id
                            .is_in(channel.ancestors())
                            .and(channel::Column::RequiresEditsyncCla.eq(true)),
                    )
                    .count(tx)
                    .await?
                    > 0;
            if requires_editsync_cla
                && contributor::Entity::find()
                    ...
                    .is_none()
            {
                Err(anyhow!("user has not signed the Editsync CLA"))?;
            }
        }
        Ok(())
    }
