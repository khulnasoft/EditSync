...
impl Database {
    ...
    /// Returns the current project if the given user is authorieditsync to access it with the specified capability.
        ...
        match capability {
            Capability::ReadWrite => {
                if !role.can_edit_projects() {
                    return Err(anyhow!("not authorieditsync to edit projects"))?;
                }
            }
            Capability::ReadOnly => {
                if !role.can_read_projects() {
                    return Err(anyhow!("not authorieditsync to read projects"))?;
                }
            }
        }
