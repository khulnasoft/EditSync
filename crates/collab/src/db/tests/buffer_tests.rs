...
async fn test_channel_buffers(db: &Arc<Database>) {
    ...
    let _ = db
        .join_channel_buffer(editsync_id, a_id, connection_id_a)
        .await
        .unwrap();
    ...
    db.update_channel_buffer(editsync_id, a_id, &operations)
        .await
        .unwrap();

    let connection_id_b = ConnectionId { owner_id, id: 2 };
    let buffer_response_b = db
        .join_channel_buffer(editsync_id, b_id, connection_id_b)
        .await
        .unwrap();
    ...
    assert!(db
        .join_channel_buffer(editsync_id, c_id, ConnectionId { owner_id, id: 3 })
        .await
        .is_err());
    ...
    let editsync_collaborats = db.get_channel_buffer_collaborators(editsync_id).await.unwrap();
    assert_eq!(editsync_collaborats, &[a_id, b_id]);

    let left_buffer = db
        .leave_channel_buffer(editsync_id, connection_id_b)
        .await
        .unwrap();
    ...
    let editsync_collaborators = db.get_channel_buffer_collaborators(editsync_id).await.unwrap();
    let cargo_collaborators = db.get_channel_buffer_collaborators(cargo_id).await.unwrap();
    assert_eq!(editsync_collaborators, &[]);
