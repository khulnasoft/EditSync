...
async fn test_channels(db: &Arc<Database>) {
    ...
    assert!(db.get_channel(editsync_id, b_id).await.is_err());

    db.invite_channel_member(editsync_id, b_id, a_id, ChannelRole::Member)
        .await
        .unwrap();

    db.respond_to_channel_invite(editsync_id, b_id, true)
        .await
        .unwrap();
    ...
    let livestreaming_id = db
        ...
        .unwrap();
    let replace_id = db
        .create_sub_channel("replace", editsync_id, a_id)
        .await
        .unwrap();
    ...
    assert_eq!(
        result.channels,
        channel_tree(&[
            (editsync_id, &[], "editsync"),
            (crdb_id, &[editsync_id], "crdb"),
            (livestreaming_id, &[editsync_id], "livestreaming",),
            (replace_id, &[editsync_id], "replace"),
            (rust_id, &[], "rust"),
    ...
    let set_channel_admin = db
        .set_channel_member_role(editsync_id, a_id, b_id, ChannelRole::Admin)
        .await;
...
async fn test_channel_renames(db: &Arc<Database>) {
    ...
    let editsync_id = db.create_root_channel("editsync", user_1).await.unwrap();

    db.rename_channel(editsync_id, user_1, "#editsync-archive")
        .await
        .unwrap();

    let channel = db.get_channel(editsync_id, user_1).await.unwrap();
    assert_eq!(channel.name, "editsync-archive");

    let non_permissioned_rename = db.rename_channel(editsync_id, user_2, "hacked-lol").await;
    assert!(non_permissioned_rename.is_err());

    let bad_name_rename = db.rename_channel(editsync_id, user_1, "#").await;
...
async fn test_db_channel_moving(db: &Arc<Database>) {
    ...
    // Initial DAG:
    //     /- gpui2
    // editsync -- crdb - livestreaming - livestreaming_dag
    let result = db.get_channels_for_user(a_id).await.unwrap();
    assert_channel_tree(
        result.channels,
        &[
...
async fn test_db_channel_moving_bugs(db: &Arc<Database>) {
    ...
    let editsync_id = db.create_root_channel("editsync", user_id).await.unwrap();

    let projects_id = db
        .create_sub_channel("projects", editsync_id, user_id)
        .await
        .unwrap();
    ...
    assert_channel_tree(
        ...
        &[
            (editsync_id, &[]),
            (projects_id, &[editsync_id]),
            (livestreaming_id, &[editsync_id, projects_id]),
        ],
    );
...
async fn test_user_is_channel_participant(db: &Arc<Database>) {
    ...
    let internal_channel_id = db
        .create_sub_channel("active", editsync_channel, admin)
        .await
        .unwrap();
    let public_channel_id = db
        .create_sub_channel("vim", editsync_channel, admin)
        .await
        .unwrap();
    ...
    .await
    .unwrap();
    db.invite_channel_member(editsync_channel, member, admin, ChannelRole::Member)
        .await
        .unwrap();
    db.invite_channel_member(editsync_channel, guest, admin, ChannelRole::Guest)
        .await
        .unwrap();

    db.respond_to_channel_invite(editsync_channel, member, true)
        .await
        .unwrap();
    ...
    db.respond_to_channel_invite(editsync_channel, guest, true)
        .await
        .unwrap();
    ...
    assert_channel_tree(
        ...
        &[
            (editsync_channel, &[]),
            (internal_channel_id, &[editsync_channel]),
            (public_channel_id, &[editsync_channel]),
        ],
    );

    db.set_channel_member_role(editsync_channel, admin, guest, ChannelRole::Banned)
        .await
        .unwrap();
    assert!(db
    ...
    db.remove_channel_member(editsync_channel, guest, admin)
        .await
        .unwrap();
...
async fn test_guest_access(db: &Arc<Database>) {
    ...
    db.join_channel(editsync_channel, guest, guest_connection)
        .await
        .unwrap();
