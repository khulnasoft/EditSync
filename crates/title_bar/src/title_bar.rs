...
const BOOK_ONBOARDING: &str = "https://dub.sh/editsync-onboarding";
...
impl TitleBar {
    pub fn new(
        ...
        let application_menu = match platform_style {
            PlatformStyle::Mac => {
                if option_env!("EDITSYNC_USE_CROSS_PLATFORM_MENU").is_some() {
        ...
        Some(
            Button::new("project_branch_trigger", branch_name)
                ...
                .tooltip(move |cx| {
                    Tooltip::with_meta(
                        "Recent Branches",
                        Some(&editsync_actions::branches::OpenRecent),
                        "Local branches only",
                        cx,
                    )
                })
                .on_click(move |_, cx| {
                    let _ = workspace.update(cx, |_this, cx| {
                        cx.dispatch_action(editsync_actions::branches::OpenRecent.boxed_clone());
                    });
                }),
        )
    }
            ...
            client::Status::UpgradeRequired => {
                ...
                let label = match auto_updater.map(|auto_update| auto_update.read(cx).status()) {
                    ...
                    Some(AutoUpdateStatus::Idle) | Some(AutoUpdateStatus::Errored) | None => {
                        "Please update Editsync to Collaborate"
                    }
                };
            ...
            PopoverMenu::new("user-menu")
                .menu(move |cx| {
                    ContextMenu::build(cx, |menu, cx| {
                        menu.when(cx.has_flag::<EditsyncPro>(), |menu| {
                            menu.action(
                                format!(
                                    "Current Plan: {}",
                                    match plan {
                                        ...
                                        Some(proto::Plan::EditsyncPro) => "Pro",
                                    }
                                ),
                                editsync_actions::OpenAccountSettings.boxed_clone(),
                            )
                            .separator()
                        })
                        .action("Settings", editsync_actions::OpenSettings.boxed_clone())
                        .action("Key Bindings", Box::new(editsync_actions::OpenKeymap))
                        .action(
                            "Themes…",
                            editsync_actions::theme_selector::Toggle::default().boxed_clone(),
                        )
                        .action("Extensions", editsync_actions::Extensions.boxed_clone())
                        .separator()
                        .link(
                            "Book Onboarding",
                            OpenBrowser {
            ...
            PopoverMenu::new("user-menu")
                .menu(|cx| {
                    ContextMenu::build(cx, |menu, _| {
                        menu.action("Settings", editsync_actions::OpenSettings.boxed_clone())
                            .action("Key Bindings", Box::new(editsync_actions::OpenKeymap))
                            .action(
                                "Themes…",
                                editsync_actions::theme_selector::Toggle::default().boxed_clone(),
