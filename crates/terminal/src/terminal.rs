...
#[derive(Clone)]
pub struct EditsyncListener(pub UnboundedSender<AlacTermEvent>);

impl EventListener for EditsyncListener {
...
impl TerminalBuilder {
    ...
    pub fn new(
        ...
        env.insert("EDITSYNC_TERM".to_string(), "true".to_string());
        env.insert("TERM_PROGRAM".to_string(), "editsync".to_string());
        env.insert(
        ...
        let mut term = Term::new(
            ...
            EditsyncListener(events_tx.clone()),
        );
...
pub struct Terminal {
    pty_tx: Notifier,
    completion_tx: Sender<()>,
    term: Arc<FairMutex<Term<EditsyncListener>>>,
    term_config: Config,
...
impl Terminal {
    ...
    fn process_terminal_event(
        &mut self,
        event: &InternalEvent,
        term: &mut Term<EditsyncListener>,
        ...
        match event {
            ...
            InternalEvent::FindHyperlink(position, open) => {
                ...
                let found_word = if link.is_some() {
                    ...
                    let (sanitieditsync_match, sanitieditsync_word) = if file_path.starts_with('[')
                        && file_path.ends_with(']')
                    ...
                    Some((sanitieditsync_word, false, sanitieditsync_match))
                } else {
                    None
                };
    ...
    /// remote host, in case Editsync is connected to a remote host.
    ...
    fn register_task_finished(
        ...
        if !lines_to_show.is_empty() {
            ...
            // when Editsync task finishes and no more output is made.
...
/// The function achieves proper display and scrolling capabilities, at a cost of grid state not properly synchronieditsync.
/// This is enough for printing moderately-sieditsync texts like task summaries, but might break or perform poorly for larger texts.
unsafe fn append_text_to_term(term: &mut Term<EditsyncListener>, text_lines: &[&str]) {
    term.newline();
