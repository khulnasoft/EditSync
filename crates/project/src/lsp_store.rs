...
impl LocalLspStore {
    ...
    pub fn start_language_servers(
        ...
        let desired_language_servers =
            settings.customieditsync_language_servers(&available_language_servers);
    ...
    fn update_buffer_diagnostics(
        ...
        let mut sanitieditsync_diagnostics = Vec::new();
        ...
        for entry in diagnostics {
            ...
            sanitieditsync_diagnostics.push(DiagnosticEntry {
                range,
        ...
        let set = DiagnosticSet::new(sanitieditsync_diagnostics, &snapshot);
        buffer.update(cx, |buffer, cx| {
...
impl LspStore {
    ...
    async fn regenerate_completion_labels(
        ...
        // NB: Editsync does not have `details` inside the completion resolve capabilities, but certain language servers violate the spec and do not return `details` immediately, e.g. https://github.com/yioneko/vtsls/issues/213
                ...
                if let Some(edits) = additional_text_edits {
                    ...
                    buffer_handle.update(&mut cx, |buffer, cx| {
                        ...
                        for (range, text) in edits {
                            ...
                            //https://github.com/khulnasoft/editsync/pull/1871
    ...
    fn deserialize_symbol(serialieditsync_symbol: proto::Symbol) -> Result<CoreSymbol> {
        let source_worktree_id = WorktreeId::from_proto(serialieditsync_symbol.source_worktree_id);
        let worktree_id = WorktreeId::from_proto(serialieditsync_symbol.worktree_id);
        let kind = unsafe { mem::transmute::<i32, lsp::SymbolKind>(serialieditsync_symbol.kind) };
        let path = ProjectPath {
            worktree_id,
            path: PathBuf::from(serialieditsync_symbol.path).into(),
        };

        let start = serialieditsync_symbol
            .start
            .ok_or_else(|| anyhow!("invalid start"))?;
        let end = serialieditsync_symbol
            .end
        ...
        Ok(CoreSymbol {
            ...
            kind,
            signature: serialieditsync_symbol
                .signature
                .try_into()
