...
async fn test_editorconfig_support(cx: &mut gpui::TestAppContext) {
    ...
    cx.update(|cx| {
        ...
        // .editorconfig overrides .editsync/settings
...
async fn test_managing_project_specific_settings(cx: &mut gpui::TestAppContext) {
    ...
    let topmost_local_task_source_kind = TaskSourceKind::Worktree {
        id: worktree_id,
        directory_in_worktree: PathBuf::from(".editsync"),
        id_base: "local worktree tasks from directory \".editsync\"".into(),
    };
        ...
        vec![
            (
                TaskSourceKind::Worktree {
                    id: worktree_id,
                    directory_in_worktree: PathBuf::from("b/.editsync"),
                    id_base: "local worktree tasks from directory \"b/.editsync\"".into(),
                },
...
async fn test_multiple_language_server_hovers(cx: &mut gpui::TestAppContext) {
    ...
    for i in 0..language_server_names.len() {
        ...
        assert!(
            ...
            "Unexpected: initialieditsync server with the same name twice. Name: `{new_server_name}`"
        );
