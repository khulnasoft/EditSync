...
pub struct NodeBinarySettings {
    ...
    ///  The path to the npm binary Editsync should use (defaults to .path/../npm)
    pub npm_path: Option<String>,
    /// If disabled, editsync will download its own copy of node.
    #[serde(default)]
...
/// SettingsObserver observers changes to .editsync/{settings, task}.json files in local worktrees
/// (or the equivalent protobuf messages from upstream) and updates local settings
/// and sends notifications downstream.
/// In ssh mode it also monitors ~/.config/editsync/{settings, task}.json and sends the content
/// upstream.
                        ...
                        Some(
                            async move {
                                ...
                                if abs_path.ends_with(local_vscode_tasks_file_relative_path()) {
                                    ...
                                    let editsync_tasks = TaskTemplates::try_from(vscode_tasks)
                                        .with_context(|| {
                                            format!(
                                        "converting VSCode tasks into Editsync ones, file {abs_path:?}"
                                    )
                                        })?;
                                    serde_json::to_string(&editsync_tasks).with_context(|| {
                                        format!(
                                            "serializing Editsync tasks into JSON, file {abs_path:?}"
                                        )
                                    })
                                } else {
                                    Ok(content)
