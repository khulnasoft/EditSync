...
impl RecentProjectsDelegate {
    ...
    pub fn set_workspaces(
        ...
        self.has_any_non_local_projects = !self
            .workspaces
            .iter()
            .all(|(_, location)| matches!(location, SerialieditsyncWorkspaceLocation::Local(_, _)));
    }
}
...
impl PickerDelegate for RecentProjectsDelegate {
        ...
        let candidates = self
            ...
            .map(|(id, (_, location))| {
                let combined_string = match location {
                    SerialieditsyncWorkspaceLocation::Local(paths, order) => order
                        .order()
                        .iter()
                        ...
                        .collect::<Vec<_>>()
                        .join(""),
                    SerialieditsyncWorkspaceLocation::Ssh(ssh_project) => ssh_project
                        .ssh_urls()
                        .iter()
                ...
                .update(cx, |workspace, cx| {
                    if workspace.database_id() == Some(*candidate_workspace_id) {
                        ...
                        match candidate_workspace_location {
                            SerialieditsyncWorkspaceLocation::Local(paths, _) => {
    ...
    fn render_match(
        ...
        let paths = match location {
            SerialieditsyncWorkspaceLocation::Local(paths, order) => Arc::new(
                order
                    .order()
                    .iter()
                    .zip(paths.paths().iter())
            ...
            SerialieditsyncWorkspaceLocation::Ssh(ssh_project) => Arc::new(ssh_project.ssh_urls()),
        };
                    ...
                    h_flex()
                        ...
                        .when(self.has_any_non_local_projects, |this| {
                            this.child(match location {
                                SerialieditsyncWorkspaceLocation::Local(_, _) => {
            ...
            .update(cx, |_, cx| {
                recent_projects_picker.update(cx, |picker, cx| {
                    ...
                    delegate.set_workspaces(vec![(
                        WorkspaceId::default(),
                        SerialieditsyncWorkspaceLocation::from_local_paths(vec!["/test/path/"]),
                    )]);
                });
            })
            .unwrap();
