...
#[derive(Clone, PartialEq, Deserialize)]
pub struct OpenEditsyncUrl {
    pub url: String,
}

impl_actions!(editsync, [OpenBrowser, OpenEditsyncUrl]);
...
pub struct Rerun {
    ...
    /// If it is not, environment variables such as EDITSYNC_COLUMN, EDITSYNC_FILE are gonna be the same as in the last execution of a task
