...
///
/// This improves the likelihood of a match by either humanieditsync name or keymap-style name.
...
impl CommandPaletteDelegate {
    ...
    fn matches_updated(
        ...
        if parse_editsync_link(&query, cx).is_some() {
            intercept_result = Some(CommandInterceptResult {
                action: OpenEditsyncUrl { url: query.clone() }.boxed_clone(),
                string: query.clone(),
                positions: vec![],
            })
        }
...
mod tests {
    ...
    #[gpui::test]
    async fn test_normalieditsync_matches(cx: &mut TestAppContext) {
        let app_state = init_test(cx);
        let project = Project::test(app_state.fs.clone(), [], cx).await;
        let (workspace, cx) = cx.add_window_view(|cx| Workspace::test_new(project.clone(), cx));
