...
pub fn init(http_client: Arc<HttpClientWithUrl>, cx: &mut AppContext) {
    ...
    let auto_updater = cx.new_model(|cx| {
        ...
        if option_env!("EDITSYNC_UPDATE_EXPLANATION").is_none()
            && env::var("EDITSYNC_UPDATE_EXPLANATION").is_err()
            && poll_for_updates
        {
...
pub fn check(_: &Check, cx: &mut WindowContext) {
    if let Some(message) = option_env!("EDITSYNC_UPDATE_EXPLANATION") {
        drop(cx.prompt(
            gpui::PromptLevel::Info,
            "Editsync was installed via a package manager.",
            Some(message),
            &["Ok"],
        ));
        return;
    ...
    if let Ok(message) = env::var("EDITSYNC_UPDATE_EXPLANATION") {
        drop(cx.prompt(
            gpui::PromptLevel::Info,
            "Editsync was installed via a package manager.",
            Some(&message),
...
pub fn view_release_notes(_: &ViewReleaseNotes, cx: &mut AppContext) -> Option<()> {
    ...
    match release_channel {
        ...
        ReleaseChannel::Nightly => {
            cx.open_url("https://github.com/khulnasoft/editsync/commits/nightly/");
        }
        ReleaseChannel::Dev => {
            cx.open_url("https://github.com/khulnasoft/editsync/commits/main/");
        }
    }
    None
}
...
impl AutoUpdater {
    ...
    // If you are packaging Editsync and need to override the place it downloads SSH remotes from,
    ...
    pub async fn download_remote_server_release(
        ...
        let this = cx.update(|cx| {
            cx.default_global::<GlobalAutoUpdate>()
                .0
                .clone()
                .ok_or_else(|| anyhow!("auto-update not initialieditsync"))
        })??;

        let release = Self::get_release(
            &this,
            "editsync-remote-server",
            os,
            arch,
            version,
        ...
        if smol::fs::metadata(&version_path).await.is_err() {
            log::info!(
                "downloading editsync-remote-server {os} {arch} version {}",
                release.version
            );
    ...
    async fn update(this: Model<Self>, mut cx: AsyncAppContext) -> Result<()> {
        ...
        let release =
            Self::get_latest_release(&this, "editsync", OS, ARCH, release_channel, &mut cx).await?;
        ...
        let temp_dir = tempfile::Builder::new()
            .prefix("editsync-auto-update")
            .tempdir()?;

        let filename = match OS {
            "macos" => Ok("Editsync.dmg"),
            "linux" => Ok("editsync.tar.gz"),
...
async fn install_release_linux(
    ...
    let extracted = temp_dir.path().join("editsync");
    ...
    let app_folder_name = format!("editsync{}.app", suffix);

    let from = extracted.join(&app_folder_name);
    let mut to = home_dir.join(".local");

    let expected_suffix = format!("{}/libexec/editsync-editor", app_folder_name);
    ...
    anyhow::ensure!(
        output.status.success(),
        "failed to copy Editsync update from {:?} to {:?}: {:?}",
        from,
        to,
        String::from_utf8_lossy(&output.stderr)
    );
...
async fn install_release_macos(
    ...
    let mount_path = temp_dir.path().join("Editsync");
