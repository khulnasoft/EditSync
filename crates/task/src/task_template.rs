...
use crate::{
    ...
    EDITSYNC_VARIABLE_NAME_PREFIX,
};

/// A template definition of a Editsync task to run.
...
/// contains all relevant Editsync state in task variables.
...
impl TaskTemplate {
    ...
    /// If any replacement fails or the new string substitutions still have [`EDITSYNC_VARIABLE_NAME_PREFIX`],
    /// `None` is returned.
    ///
...
fn substitute_all_template_variables_in_str<A: AsRef<str>>(
    ...
    let substituted_string = shellexpand::env_with_context(template_str, |var| {
        ...
        if let Some(name) = task_variables.get(variable_name) {
            ...
            return Ok(Some(name));
        } else if variable_name.starts_with(EDITSYNC_VARIABLE_NAME_PREFIX) {
...
mod tests {
    ...
    fn test_template_variables_resolution() {
        ...
        for i in 0..15 {
            ...
            assert_eq!(
                ...
                &[
                    "arg1 $EDITSYNC_SELECTED_TEXT",
                    "arg2 $EDITSYNC_COLUMN",
                    "arg3 $EDITSYNC_SYMBOL",
                ],
        ...
        for i in 0..all_variables.len() {
            ...
            assert_eq!(resolved_task_attempt, None, "If any of the Editsync task variables is not substituted, the task should not be resolved, but got some resolution without the variable {removed_variable:?} (index {i})");
        }
    }
    ...
    #[test]
    fn test_errors_on_missing_editsync_variable() {
        let task = TaskTemplate {
            label: "My task".into(),
            command: "echo".into(),
            args: vec!["$EDITSYNC_VARIABLE".into()],
            ..TaskTemplate::default()
        };
        assert!(task
            .resolve_task(TEST_ID_BASE, &TaskContext::default())
