...
type EditsyncEnvVariable = String;

struct EnvVariableReplacer {
    variables: HashMap<VsCodeEnvVariable, EditsyncEnvVariable>,
}

impl EnvVariableReplacer {
    fn new(variables: HashMap<VsCodeEnvVariable, EditsyncEnvVariable>) -> Self {
        Self { variables }
    }
    // Replaces occurrences of VsCode-specific environment variables with Editsync equivalents.
    fn replace(&self, input: &str) -> String {
        shellexpand::env_with_context_no_errors(&input, |var: &str| {
            ...
            if let Some(substitution) = self.variables.get(variable_name) {
                // Got a VSCode->Editsync hit, perform a substitution
...
impl VsCodeTaskDefinition {
    fn into_editsync_format(self, replacer: &EnvVariableReplacer) -> anyhow::Result<TaskTemplate> {
...
mod tests {
    ...
    fn test_variable_substitution() {
        ...
        let replacer = EnvVariableReplacer::new(HashMap::from_iter([(
            "PATH".to_owned(),
            "EDITSYNC_PATH".to_owned(),
        )]));
        assert_eq!(replacer.replace("Food"), "Food");
        assert_eq!(
            replacer.replace("$PATH is an environment variable"),
            "${EDITSYNC_PATH} is an environment variable"
        );
        assert_eq!(replacer.replace("${PATH}"), "${EDITSYNC_PATH}");
        assert_eq!(replacer.replace("${PATH:food}"), "${EDITSYNC_PATH:food}");
    }
    ...
    fn can_deserialize_ts_tasks() {
        ...
        let expected = vec![
            ...
            TaskTemplate {
                ...
                command: "node".to_string(),
                args: vec![
                    "${EDITSYNC_WORKTREE_ROOT}/node_modules/typescript/lib/tsc.js".to_string(),
                    "--build".to_string(),
                    "${EDITSYNC_WORKTREE_ROOT}/src".to_string(),
                    "--watch".to_string(),
                ],
                ..Default::default()
            },
