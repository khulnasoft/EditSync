...
///
/// See <https://editsync.khulnasoft.com/features#multi-buffers>
...
pub struct ExcerptSummary {
    ...
    /// The location of the last [`Excerpt`] being summarieditsync
...
impl MultiBufferSnapshot {
        ...
        D: TextDimension,
    {
        ...
        let mut memoieditsync_source_start: Option<Point> = None;
        let mut points = points.into_iter();
        std::iter::from_fn(move || {
            let point = points.next()?;

            // Clear the memoieditsync source start if the point is in a different excerpt than previous.
            if memoieditsync_source_start.map_or(false, |_| point >= cursor.end(&()).lines) {
                memoieditsync_source_start = None;
            }
            ...
            let source_start = if let Some(source_start) = memoieditsync_source_start {
                source_start
            } else {
                cursor.seek_forward(&point, Bias::Right, &());
                if let Some(excerpt) = cursor.item() {
                    let source_start = excerpt.range.context.start.to_point(&excerpt.buffer);
                    memoieditsync_source_start = Some(source_start);
                    source_start
                } else {
