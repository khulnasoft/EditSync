...
struct SerialieditsyncProjectPanel {
    width: Option<Pixels>,
}
...
impl ProjectPanel {
    ...
    pub async fn load(
        ...
        let serialieditsync_panel = cx
            ...
            .log_err()
            .flatten()
            .map(|panel| serde_json::from_str::<SerialieditsyncProjectPanel>(&panel))
            .transpose()
            .log_err()
            .flatten();
        ...
        workspace.update(&mut cx, |workspace, cx| {
            ...
            if let Some(serialieditsync_panel) = serialieditsync_panel {
                panel.update(cx, |panel, cx| {
                    panel.width = serialieditsync_panel.width.map(|px| px.round());
                    cx.notify();
                });
            }
            panel
            ...
            async move {
                KEY_VALUE_STORE
                    .write_kvp(
                        ...
                        serde_json::to_string(&SerialieditsyncProjectPanel { width })?,
                    )
                    .await?;
                anyhow::Ok(())
            }
    ...
    fn find_next_selection_after_deletion(
        &self,
        sanitieditsync_entries: BTreeSet<SelectedEntry>,
        cx: &mut ViewContext<Self>,
    ) -> Option<SelectedEntry> {
        if sanitieditsync_entries.is_empty() {
            return None;
        }

        let project = self.project.read(cx);
        let (worktree_id, worktree) = sanitieditsync_entries
            .iter()
        ...
        let marked_entries_in_worktree = sanitieditsync_entries
            .iter()
    ...
    fn disjoint_entries(&self, cx: &AppContext) -> BTreeSet<SelectedEntry> {
        ...
        let mut sanitieditsync_entries = BTreeSet::new();
        if marked_entries.is_empty() {
            return sanitieditsync_entries;
        }
        ...
        for (worktree_id, marked_entries) in marked_entries_by_worktree {
            if let Some(worktree) = project.worktree_for_id(worktree_id, cx) {
                ...
                sanitieditsync_entries.extend(marked_entries.into_iter().filter(|entry| {
