...
impl ModelContextProtocol {
    ...
    pub async fn initialize(
    ...
    ) -> Result<InitialieditsyncContextServerProtocol> {
        ...
        self.inner.notify(
            types::NotificationType::Initialieditsync.as_str(),
            serde_json::json!({}),
        )?;

        let initialieditsync_protocol = InitialieditsyncContextServerProtocol {
            inner: self.inner,
            initialize: response,
        };

        Ok(initialieditsync_protocol)
    }
}

pub struct InitialieditsyncContextServerProtocol {
    inner: Client,
...
impl InitialieditsyncContextServerProtocol {
