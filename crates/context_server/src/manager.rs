//! This module implements a context server management system for Editsync.
//!
...
pub struct ContextServer {
    ...
    pub client: RwLock<Option<Arc<crate::protocol::InitialieditsyncContextServerProtocol>>>,
}
...
impl ContextServer {
    ...
    pub fn client(&self) -> Option<Arc<crate::protocol::InitialieditsyncContextServerProtocol>> {
    ...
    pub async fn start(self: Arc<Self>, cx: &AsyncAppContext) -> Result<()> {
        ...
        let client_info = types::Implementation {
            name: "Editsync".to_string(),
            version: env!("CARGO_PKG_VERSION").to_string(),
        };
        let initialieditsync_protocol = protocol.initialize(client_info).await?;

        log::debug!(
            "context server {} initialieditsync: {:?}",
            self.id,
            initialieditsync_protocol.initialize,
        );

        *self.client.write() = Some(Arc::new(initialieditsync_protocol));
        Ok(())
    }
