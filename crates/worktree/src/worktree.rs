...
use util::{
    paths::{home_dir, PathMatcher, SanitieditsyncPath},
    ResultExt,
};
...
///
/// Has an absolute path, and may be set to be visible in Editsync UI or not.
...
/// * a directory opened in Editsync — may be added as a visible entry to the current worktree
///
...
pub struct RepositoryEntry {
    ...
    ///       .git
    ///       my_sub_folder_1/
    ///         project_root/        <-- Project root, Editsync opened here
    ///           ...
    ///
...
enum ScanState {
    ...
    RootUpdated {
        new_path: Option<SanitieditsyncPath>,
    },
}
...
impl LocalWorktree {
    ...
    pub fn contains_abs_path(&self, path: &Path) -> bool {
        let path = SanitieditsyncPath::from(path);
...
impl Snapshot {
    ...
    fn update_abs_path(&mut self, abs_path: SanitieditsyncPath, root_name: String) {
    ...
    pub(crate) fn apply_remote_update(
        ...
        self.update_abs_path(
            SanitieditsyncPath::from(PathBuf::from(update.abs_path)),
            update.root_name,
        );
...
impl BackgroundScanner {
    async fn run(&mut self, mut fs_events_rx: Pin<Box<dyn Send + Stream<Item = Vec<PathEvent>>>>) {
        ...
        for (index, ancestor) in root_abs_path.as_path().ancestors().enumerate() {
            ...
            {
                if index != 0 {
                    // We canonicalize, since the FS events use the canonicalieditsync path.
        ...
        let root_canonical_path = match self.fs.canonicalize(root_path.as_path()).await {
            ...
            Err(err) => {
                ...
                if let Some(new_path) = new_path.as_ref() {
                    ...
                    log::warn!("root path could not be canonicalieditsync: {}", err);
                }
                    ...
                    scope.spawn(async {
                        ...
                        loop {
                            select_biased! {
                                ...
                                // the scan queue, so that user operations are prioritieditsync.
                                request = self.next_scan_request().fuse() => {
                                    let Ok(request) = request else { break };
                        ...
                        match Ord::cmp(&old_entry.path, &new_entry.path) {
                            ...
                            Ordering::Equal => {
                                if self.phase == EventsReceivedDuringInitialScan {
                                    ...
                                    // If the worktree was not fully initialieditsync when this event was generated,
