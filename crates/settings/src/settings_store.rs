...
pub trait Settings: 'static + Send + Sync {
    ...
    /// be deserialieditsync. If this is `None`, then the setting will be deserialieditsync
...
trait AnySettingValue: 'static + Send + Sync {
    ...
    fn deserialize_setting(&self, json: &serde_json::Value) -> Result<DeserialieditsyncSetting>;
    fn load_setting(
        &self,
        sources: SettingsSources<DeserialieditsyncSetting>,
        cx: &mut AppContext,
    ) -> Result<Box<dyn Any>>;
...
struct DeserialieditsyncSetting(Box<dyn Any>);
...
impl SettingsStore {
    ...
    /// Configure the tab sieditsync when updating JSON files.
    ...
    pub fn set_local_settings(
        ...
        let mut editsync_settings_changed = false;
        match (
            kind,
            settings_content
            ...
            (LocalSettingsKind::Settings, None) => {
                editsync_settings_changed = self
                    .raw_local_settings
            ...
            (LocalSettingsKind::Settings, Some(settings_contents)) => {
                    ...
                    .entry((root_id, directory_path.clone()))
                {
                    btree_map::Entry::Vacant(v) => {
                        ...
                        editsync_settings_changed = true;
                    }
    ...
    fn recompute_values(
        ...
        let mut project_settings_stack = Vec::<DeserialieditsyncSetting>::new();
...
impl<T: Settings> AnySettingValue for SettingValue<T> {
    ...
    fn load_setting(
        &self,
        values: SettingsSources<DeserialieditsyncSetting>,
        cx: &mut AppContext,
    ) -> Result<Box<dyn Any>> {
        Ok(Box::new(T::load(
            SettingsSources {
    ...
    fn deserialize_setting(
        ...
        if let Some(key) = T::KEY {
            if let Some(value) = json.get(key) {
                ...
                return Ok(DeserialieditsyncSetting(Box::new(value)));
            }
        }
        let value = T::FileContent::deserialize(json)?;
        Ok(DeserialieditsyncSetting(Box::new(value)))
    }
