...
/// Due to the issue of UNC paths on Windows, which can cause bugs in various parts of Editsync, introducing this `SanitieditsyncPath`
/// leverages Rust's type system to ensure that all paths entering Editsync are always "sanitieditsync" by removing the `\\\\?\\` prefix.
/// On non-Windows operating systems, this struct is effectively a no-op.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct SanitieditsyncPath(Arc<Path>);

impl SanitieditsyncPath {
    pub fn starts_with(&self, prefix: &SanitieditsyncPath) -> bool {
...
impl From<SanitieditsyncPath> for Arc<Path> {
    fn from(sanitieditsync_path: SanitieditsyncPath) -> Self {
        sanitieditsync_path.0
    }
}

impl<T: AsRef<Path>> From<T> for SanitieditsyncPath {
    #[cfg(not(target_os = "windows"))]
    fn from(path: T) -> Self {
        let path = path.as_ref();
        SanitieditsyncPath(path.into())
    }

    #[cfg(target_os = "windows")]
    fn from(path: T) -> Self {
        let path = path.as_ref();
        SanitieditsyncPath(dunce::simplified(path).into())
    }
}
...
mod tests {
    ...
    fn path_with_position_parse_posix_path_with_suffix() {
        assert_eq!(
            PathWithPosition::parse_str("app-editors:editsync-0.143.6:20240710-201212.log:34:"),
            PathWithPosition {
                path: PathBuf::from("app-editors:editsync-0.143.6:20240710-201212.log"),
                row: Some(34),
                column: None,
            }
        );
    ...
    #[test]
    #[cfg(target_os = "windows")]
    fn test_sanitieditsync_path() {
        let path = Path::new("C:\\Users\\someone\\test_file.rs");
        let sanitieditsync_path = SanitieditsyncPath::from(path);
        assert_eq!(
            sanitieditsync_path.to_string(),
