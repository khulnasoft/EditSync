...
pub fn load_login_shell_environment() -> Result<()> {
    let marker = "EDITSYNC_LOGIN_SHELL_START";
...
pub fn measure<R>(label: &str, f: impl FnOnce() -> R) -> R {
    static EDITSYNC_MEASUREMENTS: OnceLock<bool> = OnceLock::new();
    let editsync_measurements = EDITSYNC_MEASUREMENTS.get_or_init(|| {
        env::var("EDITSYNC_MEASUREMENTS")
            .map(|measurements| measurements == "1" || measurements == "true")
            .unwrap_or(false)
    });

    if *editsync_measurements {
