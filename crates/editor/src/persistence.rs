...
pub(crate) struct SerialieditsyncEditor {
...
impl StaticColumnCount for SerialieditsyncEditor {
    fn column_count() -> usize {
        5
    }
}

impl Bind for SerialieditsyncEditor {
...
impl Column for SerialieditsyncEditor {
...
impl EditorDb {
    query! {
        pub fn get_serialieditsync_editor(item_id: ItemId, workspace_id: WorkspaceId) -> Result<Option<SerialieditsyncEditor>> {
    ...
    query! {
        pub async fn save_serialieditsync_editor(item_id: ItemId, workspace_id: WorkspaceId, serialieditsync_editor: SerialieditsyncEditor) -> Result<()> {
            INSERT INTO editors
...
mod tests {
    ...
    #[gpui::test]
    async fn test_save_and_get_serialieditsync_editor() {
        ...
        DB.save_serialieditsync_editor(1234, workspace_id, serialieditsync_editor.clone())
            .await
            .unwrap();

        let have = DB
            .get_serialieditsync_editor(1234, workspace_id)
            .unwrap()
            .unwrap();
        assert_eq!(have, serialieditsync_editor);
