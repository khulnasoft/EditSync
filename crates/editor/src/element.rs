...
impl EditorElement {
    ...
    fn render_block(
        ...
        resieditsync_blocks: &mut HashMap<CustomBlockId, u32>,
        ...
        let quantieditsync_height = (preliminary_size.height / line_height).ceil() * line_height;
        let final_size = if preliminary_size.height == quantieditsync_height {
            preliminary_size
        } else {
            element.layout_as_root(size(available_width, quantieditsync_height.into()), cx)
        };
        ...
        if let BlockId::Custom(custom_block_id) = block_id {
            if block.height() > 0 {
                ...
                if element_height_in_lines != block.height() {
                    resieditsync_blocks.insert(custom_block_id, element_height_in_lines);
                }
            }
        }
    ...
    fn render_blocks(
        ...
        let mut resieditsync_blocks = HashMap::default();
            ...
            cx.with_text_style(Some(text_style), |cx| {
                cx.with_content_mask(Some(ContentMask { bounds }), |cx| {
                    ...
                    let mut blocks = match blocks {
                        ...
                        Err(resieditsync_blocks) => {
                            self.editor.update(cx, |editor, cx| {
                                editor.resize_blocks(resieditsync_blocks, autoscroll_request, cx)
                            });
