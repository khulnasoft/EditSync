...
async fn test_toggle_block_comment(cx: &mut gpui::TestAppContext) {
    ...
    cx.executor().run_until_parked();
    cx.update_editor(|editor, cx| editor.toggle_comments(&ToggleComments::default(), cx));
    // TODO this is how it actually worked in Editsync Stable, which is not very ergonomic.
...
fn test_refresh_selections_while_selecting_with_mouse(cx: &mut TestAppContext) {
    ...
    _ = editor.update(cx, |editor, cx| {
        ...
        // Ensure we don't panic when selections are refreshed and that the pending selection is finalieditsync.
        editor.change_selections(None, cx, |s| s.refresh());
        assert_eq!(
            editor.selections.ranges(cx),
            [Point::new(0, 3)..Point::new(0, 3)]
...
struct Row10;"#};
    ...
    // Deletion hunks are ephemeral, so it's impossible to place the caret into them — Editsync triggers reverts for lines, adjacent to carets and selections.
    assert_hunk_revert(
        indoc! {r#"struct Row;
                   ˇstruct Row2;
