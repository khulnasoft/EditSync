...
mod tests {
    ...
    async fn test_urls(cx: &mut gpui::TestAppContext) {
        ...
        cx.set_state(indoc! {"
            Let's test a [complex](https://editsync.khulnasoft.com/channel/had-(oops)) caseˇ.
        "});

        let screen_coord = cx.pixel_position(indoc! {"
            Let's test a [complex](https://editsync.khulnasoft.com/channel/had-(ˇoops)) case.
            "});

        cx.simulate_mouse_move(screen_coord, None, Modifiers::secondary_key());
        cx.assert_editor_text_highlights::<HoveredLinkState>(indoc! {"
            Let's test a [complex](«https://editsync.khulnasoft.com/channel/had-(oops)ˇ») case.
        "});

        cx.simulate_click(screen_coord, Modifiers::secondary_key());
        assert_eq!(
            cx.opened_url(),
            Some("https://editsync.khulnasoft.com/channel/had-(oops)".into())
        );
    }
    ...
    async fn test_urls_at_beginning_of_buffer(cx: &mut gpui::TestAppContext) {
        ...
        cx.assert_editor_text_highlights::<HoveredLinkState>(
            indoc! {"«https://editsync.khulnasoft.com/releasesˇ» is a cool webpage."},
        );
    ...
    async fn test_urls_at_end_of_buffer(cx: &mut gpui::TestAppContext) {
        ...
        cx.simulate_mouse_move(screen_coord, None, Modifiers::secondary_key());
        cx.assert_editor_text_highlights::<HoveredLinkState>(
            indoc! {"A cool webpage is «https://editsync.khulnasoft.com/releasesˇ»"},
        );
