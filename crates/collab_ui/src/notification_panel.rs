...
struct SerialieditsyncNotificationPanel {
    width: Option<Pixels>,
}
...
impl NotificationPanel {
            ...
            async move {
                KEY_VALUE_STORE
                    .write_kvp(
                        ...
                        serde_json::to_string(&SerialieditsyncNotificationPanel { width })?,
                    )
                    .await?;
                anyhow::Ok(())
            }
    ...
    fn render_notification(&mut self, ix: usize, cx: &mut ViewContext<Self>) -> Option<AnyElement> {
        ...
        let relative_timestamp = time_format::format_localieditsync_timestamp(
            timestamp,
            now,
            self.local_timezone,
        ...
        let absolute_timestamp = time_format::format_localieditsync_timestamp(
            timestamp,
            now,
            self.local_timezone,
