...
struct SerialieditsyncCollabPanel {
    width: Option<Pixels>,
...
impl CollabPanel {
    ...
    pub async fn load(
        ...
        let serialieditsync_panel = cx
            ...
            .log_err()
            .flatten()
            .map(|panel| serde_json::from_str::<SerialieditsyncCollabPanel>(&panel))
            .transpose()
            .log_err()
            .flatten();
        ...
        workspace.update(&mut cx, |workspace, cx| {
            ...
            if let Some(serialieditsync_panel) = serialieditsync_panel {
                panel.update(cx, |panel, cx| {
                    ...
                    panel.collapsed_channels = serialieditsync_panel
                        .collapsed_channels
            ...
            async move {
                KEY_VALUE_STORE
                    .write_kvp(
                        ...
                        serde_json::to_string(&SerialieditsyncCollabPanel {
                            width,
                    ...
                    cx.handler_for(&this, move |_, cx| {
                        ActiveCall::global(cx)
                            ...
                            .detach_and_prompt_err("Failed to grant write access", cx, |e, _| {
                                match e.error_code() {
                                    ErrorCode::NeedsCla => Some("This user has not yet signed the CLA at https://editsync.khulnasoft.com/cla.".into()),
                                    _ => None,
                                }
                            })
                    }),
