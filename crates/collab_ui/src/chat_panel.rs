...
struct SerialieditsyncChatPanel {
    width: Option<Pixels>,
}
...
impl ChatPanel {
    ...
    pub fn load(
        ...
        cx.spawn(|mut cx| async move {
            let serialieditsync_panel = if let Some(panel) = cx
                .background_executor()
                ...
                .flatten()
            {
                Some(serde_json::from_str::<SerialieditsyncChatPanel>(&panel)?)
            } else {
                None
            };
            ...
            workspace.update(&mut cx, |workspace, cx| {
                ...
                if let Some(serialieditsync_panel) = serialieditsync_panel {
                    panel.update(cx, |panel, cx| {
                        panel.width = serialieditsync_panel.width.map(|r| r.round());
                        cx.notify();
                    });
                }
                panel
        ...
        self.pending_serialization = cx.background_executor().spawn(
            async move {
                KEY_VALUE_STORE
                    .write_kvp(
                        CHAT_PANEL_KEY.into(),
                        serde_json::to_string(&SerialieditsyncChatPanel { width })?,
                    )
                    .await?;
                anyhow::Ok(())
            }
                        ...
                        |this| {
                            this.child(
                                h_flex()
                                    ...
                                    .child(
                                        Label::new(time_format::format_localieditsync_timestamp(
                                            message.timestamp,
    ...
    fn render_markdown_with_mentions(
        ...
        if message.edited_at.is_some() {
            ...
            if let Some(edit_timestamp) = message.edited_at {
                let edit_timestamp_text = time_format::format_localieditsync_timestamp(
                    edit_timestamp,
                    OffsetDateTime::now_utc(),
                    local_timezone,
