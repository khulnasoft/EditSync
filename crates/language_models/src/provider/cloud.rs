...
use client::{
    editsync_urls, Client, PerformCompletionParams, UserStore, EXPIRED_LLM_TOKEN_HEADER_NAME,
...
use language_model::{
    ...
    LanguageModelRequest, RateLimiter, EDITSYNC_CLOUD_PROVIDER_ID,
};
use language_model::{
...
pub const PROVIDER_NAME: &str = "Editsync";

const EDITSYNC_CLOUD_PROVIDER_ADDITIONAL_MODELS_JSON: Option<&str> =
    option_env!("EDITSYNC_CLOUD_PROVIDER_ADDITIONAL_MODELS_JSON");

fn editsync_cloud_provider_additional_models() -> &'static [AvailableModel] {
    static ADDITIONAL_MODELS: LazyLock<Vec<AvailableModel>> = LazyLock::new(|| {
        EDITSYNC_CLOUD_PROVIDER_ADDITIONAL_MODELS_JSON
...
pub struct EditsyncDotDevSettings {
...
impl LanguageModelProvider for CloudLanguageModelProvider {
    fn id(&self) -> LanguageModelProviderId {
        LanguageModelProviderId(EDITSYNC_CLOUD_PROVIDER_ID.into())
    }
    ...
    fn provided_models(&self, cx: &AppContext) -> Vec<Arc<dyn LanguageModel>> {
        ...
        let llm_closed_beta_models = if cx.has_flag::<LlmClosedBeta>() {
            editsync_cloud_provider_additional_models()
        } else {
            &[]
        };
    ...
    fn render_accept_terms(&self, cx: &mut WindowContext) -> Option<AnyElement> {
        ...
        let terms = [(
            "terms_of_service",
            "Terms of Service",
            "https://editsync.khulnasoft.com/terms-of-service",
        )]
        .map(|(id, label, url)| {
            Button::new(id, label)
...
impl LanguageModel for CloudLanguageModel {
    ...
    fn telemetry_id(&self) -> String {
        format!("editsync.khulnasoft.com/{}", self.model.id())
    }
...
impl ConfigurationView {
    ...
    fn render_accept_terms(&mut self, cx: &mut ViewContext<Self>) -> Option<AnyElement> {
        ...
        let terms_button = Button::new("terms_of_service", "Terms of Service")
            ...
            .icon(IconName::ExternalLink)
            .icon_color(Color::Muted)
            .on_click(move |_, cx| cx.open_url("https://editsync.khulnasoft.com/terms-of-service"));

        let text =
            "In order to use Editsync AI, please read and accept our terms and conditions to continue:";
...
impl Render for ConfigurationView {
    fn render(&mut self, cx: &mut ViewContext<Self>) -> impl IntoElement {
        const EDITSYNC_AI_URL: &str = "https://editsync.khulnasoft.com/ai";

        let is_connected = !self.state.read(cx).is_signed_out();
        let plan = self.state.read(cx).user_store.read(cx).current_plan();
        let has_accepted_terms = self.state.read(cx).has_accepted_terms_of_service(cx);

        let is_pro = plan == Some(proto::Plan::EditsyncPro);
        let subscription_text = Label::new(if is_pro {
            "You have full access to Editsync's hosted LLMs, which include models from Anthropic, OpenAI, and Google. They come with faster speeds and higher limits through Editsync Pro."
        } else {
            "You have basic access to models from Anthropic through the Editsync AI Free plan."
        });
                ...
                h_flex()
                    ...
                    .child(
                        Button::new("learn_more", "Learn more")
                            .style(ButtonStyle::Subtle)
                            .on_click(cx.listener(|_, _, cx| cx.open_url(EDITSYNC_AI_URL))),
                    )
                    .child(
