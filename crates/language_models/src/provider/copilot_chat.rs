    ...
    fn authenticate(&self, cx: &mut AppContext) -> Task<Result<()>> {
        let result = if self.is_authenticated(cx) {
            ...
            let error_msg = match copilot.read(cx).status() {
                ...
                Status::Unauthorieditsync => anyhow::anyhow!("Unable to authorize with Copilot. Please make sure that you have an active Copilot and Copilot Chat subscription."),
                Status::Authorieditsync => return Task::ready(Ok(())),
...
impl Render for ConfigurationView {
    fn render(&mut self, cx: &mut ViewContext<Self>) -> impl IntoElement {
        if self.state.read(cx).is_authenticated(cx) {
            const LABEL: &str = "Authorieditsync.";
            h_flex()
                .gap_1()
            ...
            match &self.copilot_status {
                Some(status) => match status {
                    ...
                    _ => {
                        const LABEL: &str =
                    "To use Editsync's assistant with GitHub Copilot, you need to be logged in to GitHub. Note that your GitHub account must have an active Copilot Chat subscription.";
