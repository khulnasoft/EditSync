...
use language_model::{LanguageModelProviderId, LanguageModelRegistry, EDITSYNC_CLOUD_PROVIDER_ID};
        ...
        LanguageModelRegistry::global(cx).update(cx, move |registry, cx| {
            if enabled {
                ...
                registry.unregister_provider(
                    LanguageModelProviderId::from(EDITSYNC_CLOUD_PROVIDER_ID.to_string()),
                    cx,
                );
            }
        });
