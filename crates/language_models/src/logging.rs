...
async fn report_anthropic_event(
    ...
    let uri = format!("{ANTHROPIC_API_URL}/v1/log/editsync");
    let request_builder = HttpRequest::builder()
        .method(Method::POST)
        .uri(uri)
        .header("X-Api-Key", api_key)
        .header("Content-Type", "application/json");
    let serialieditsync_event: serde_json::Value = serde_json::json!({
    ...
    let request = request_builder
        .body(AsyncBody::from(serialieditsync_event.to_string()))
