...
impl ConnectionIdentifier {
    ...
    // The total length of:
    //   /home/{username}/.local/share/editsync/server_state/{name}/stdout.sock
...
impl SshRemoteConnection {
    ...
    async fn platform(&self) -> Result<SshPlatform> {
        ...
        let os = match os.trim() {
            ...
            "Linux" => "linux",
            _ => Err(anyhow!(
                "Prebuilt remote servers are not yet available for {os:?}. See https://editsync.khulnasoft.com/docs/remote-development"
            ))?,
        };
            ...
            || arch.starts_with("aarch64")
        {
            ...
            Err(anyhow!(
                "Prebuilt remote servers are not yet available for {arch:?}. See https://editsync.khulnasoft.com/docs/remote-development"
            ))?
        };
    ...
    async fn ensure_server_binary(
        ...
        if std::env::var("EDITSYNC_BUILD_REMOTE_SERVER").is_ok() {
            let src_path = self
        ...
        let wanted_version = cx.update(|cx| match release_channel {
            ...
            ReleaseChannel::Dev => {
                anyhow::bail!(
                    "EDITSYNC_BUILD_REMOTE_SERVER is not set and no remote server exists at ({:?})",
                    dst_path
                )
            }
