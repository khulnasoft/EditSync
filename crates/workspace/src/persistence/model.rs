use super::{SerialieditsyncAxis, SerialieditsyncWindowBounds};
use crate::{
...
pub struct SerialieditsyncSshProject {
    pub id: SshProjectId,
    pub host: String,
    pub port: Option<u16>,
...
impl SerialieditsyncSshProject {
...
impl StaticColumnCount for SerialieditsyncSshProject {
    fn column_count() -> usize {
        5
    }
}

impl Bind for &SerialieditsyncSshProject {
...
impl Column for SerialieditsyncSshProject {
...
impl LocalPaths {
    pub fn new<P: AsRef<Path>>(paths: impl IntoIterator<Item = P>) -> Self {
        let mut paths: Vec<PathBuf> = paths
            ...
            .collect();
        // Ensure all future `editsync workspace1 workspace2` and `editsync workspace2 workspace1` calls are using the same workspace.
...
#[derive(Debug, PartialEq, Clone)]
pub enum SerialieditsyncWorkspaceLocation {
    Local(LocalPaths, LocalPathsOrder),
    Ssh(SerialieditsyncSshProject),
}

impl SerialieditsyncWorkspaceLocation {
    /// Create a new `SerialieditsyncWorkspaceLocation` from a list of local paths.
    ///
    ...
    /// ```
    /// use std::path::Path;
    /// use editsync_workspace::SerialieditsyncWorkspaceLocation;
    ///
    /// let location = SerialieditsyncWorkspaceLocation::from_local_paths(vec![
    ///     Path::new("path/to/workspace1"),
    ///     Path::new("path/to/workspace2"),
    /// ]);
    /// assert_eq!(location, SerialieditsyncWorkspaceLocation::Local(
    ///    LocalPaths::new(vec![
    ///         Path::new("path/to/workspace1"),
    ///         Path::new("path/to/workspace2"),
    ///    ]),
...
#[derive(Debug, PartialEq, Clone)]
pub(crate) struct SerialieditsyncWorkspace {
    pub(crate) id: WorkspaceId,
    pub(crate) location: SerialieditsyncWorkspaceLocation,
    pub(crate) center_group: SerialieditsyncPaneGroup,
    pub(crate) window_bounds: Option<SerialieditsyncWindowBounds>,
...
#[derive(Debug, PartialEq, Clone)]
pub(crate) enum SerialieditsyncPaneGroup {
    Group {
        axis: SerialieditsyncAxis,
        flexes: Option<Vec<f32>>,
        children: Vec<SerialieditsyncPaneGroup>,
    },
    Pane(SerialieditsyncPane),
}

#[cfg(test)]
impl Default for SerialieditsyncPaneGroup {
    fn default() -> Self {
        Self::Pane(SerialieditsyncPane {
            children: vec![SerialieditsyncItem::default()],
            active: false,
            pinned_count: 0,
        })
    }
...
impl SerialieditsyncPaneGroup {
    ...
    pub(crate) async fn deserialize(
        ...
        match self {
            ...
            SerialieditsyncPaneGroup::Pane(serialieditsync_pane) => {
                let pane = workspace
                    .update(cx, |workspace, cx| workspace.add_pane(cx).downgrade())
                    .log_err()?;
                let active = serialieditsync_pane.active;
                let new_items = serialieditsync_pane
...
pub struct SerialieditsyncPane {
    pub(crate) active: bool,
    pub(crate) children: Vec<SerialieditsyncItem>,
    pub(crate) pinned_count: usize,
}

impl SerialieditsyncPane {
    pub fn new(children: Vec<SerialieditsyncItem>, active: bool, pinned_count: usize) -> Self {
        SerialieditsyncPane {
            children,
            active,
            pinned_count,
...
pub struct SerialieditsyncItem {
    pub kind: Arc<str>,
    pub item_id: ItemId,
    pub active: bool,
    pub preview: bool,
...
#[cfg(test)]
impl Default for SerialieditsyncItem {
    fn default() -> Self {
        SerialieditsyncItem {
...
impl StaticColumnCount for SerialieditsyncItem {
    fn column_count() -> usize {
        4
    }
}
impl Bind for &SerialieditsyncItem {
    fn bind(&self, statement: &Statement, start_index: i32) -> Result<i32> {
...
impl Column for SerialieditsyncItem {
    fn column(statement: &mut Statement, start_index: i32) -> Result<(Self, i32)> {
...
mod tests {
    ...
    fn test_serialize_local_paths() {
        let paths = vec!["b", "a", "c"];
        let serialieditsync = SerialieditsyncWorkspaceLocation::from_local_paths(paths);
