...
pub(crate) struct SerialieditsyncAxis(pub(crate) gpui::Axis);
impl sqlez::bindable::StaticColumnCount for SerialieditsyncAxis {}
impl sqlez::bindable::Bind for SerialieditsyncAxis {
    fn bind(
        &self,
...
impl sqlez::bindable::Column for SerialieditsyncAxis {
    fn column(
        ...
        String::column(statement, start_index).and_then(|(axis_text, next_index)| {
            Ok((
                match axis_text.as_str() {
                    ...
                    _ => anyhow::bail!("Stored serialieditsync item kind is incorrect"),
                },
                next_index,
            ))
        })
...
pub(crate) struct SerialieditsyncWindowBounds(pub(crate) WindowBounds);

impl StaticColumnCount for SerialieditsyncWindowBounds {
    fn column_count() -> usize {
        5
    }
}

impl Bind for SerialieditsyncWindowBounds {
            ...
            WindowBounds::Windowed(bounds) => {
                ...
                statement.bind(
                    &(
                        ...
                        SerialieditsyncPixels(bounds.origin.y),
                        SerialieditsyncPixels(bounds.size.width),
                        SerialieditsyncPixels(bounds.size.height),
                    ),
                    next_index,
                )
            }
...
impl Column for SerialieditsyncWindowBounds {
...
struct SerialieditsyncPixels(gpui::Pixels);
impl sqlez::bindable::StaticColumnCount for SerialieditsyncPixels {}

impl sqlez::bindable::Bind for SerialieditsyncPixels {
    fn bind(
        &self,
        statement: &sqlez::statement::Statement,
        start_index: i32,
...
define_connection! {
    ...
    &[
        ...
        CREATE TABLE workspaces(
            ...
            dock_visible INTEGER, // Deprecated. Preserving so users can downgrade Editsync.
            dock_anchor TEXT, // Deprecated. Preserving so users can downgrade Editsync.
            dock_pane INTEGER, // Deprecated.  Preserving so users can downgrade Editsync.
    ...
    // Preserving so users can downgrade Editsync.
    sql!(
...
impl WorkspaceDb {
    /// Returns a serialieditsync workspace for the given worktree_roots. If the passed array
    /// is empty, the most recent workspace is returned instead. If no workspace for the
    /// passed roots is stored, returns none.
    pub(crate) fn workspace_for_roots<P: AsRef<Path>>(
        &self,
        worktree_roots: &[P],
    ) -> Option<SerialieditsyncWorkspace> {
        ...
        let (
            ...
            Option<SerialieditsyncWindowBounds>,
            Option<Uuid>,
            Option<bool>,
            DockStructure,
            Option<u64>,
        ...
        let location = match local_paths_order {
            Some(order) => SerialieditsyncWorkspaceLocation::Local(local_paths, order),
            None => {
                let order = LocalPathsOrder::default_for_paths(&local_paths);
                SerialieditsyncWorkspaceLocation::Local(local_paths, order)
            }
        };

        Some(SerialieditsyncWorkspace {
            id: workspace_id,
            location,
            center_group: self
    ...
    pub(crate) fn workspace_for_ssh_project(
        &self,
        ssh_project: &SerialieditsyncSshProject,
    ) -> Option<SerialieditsyncWorkspace> {
        ...
        Some(SerialieditsyncWorkspace {
            id: workspace_id,
            location: SerialieditsyncWorkspaceLocation::Ssh(ssh_project.clone()),
            center_group: self
                .get_center_pane_group(workspace_id)
                .context("Getting center group")
                .log_err()?,
    ...
    pub(crate) async fn save_workspace(&self, workspace: SerialieditsyncWorkspace) {
        self.write(move |conn| {
            conn.with_savepoint("update_worktrees", || {
                ...
                match workspace.location {
                    SerialieditsyncWorkspaceLocation::Local(local_paths, local_paths_order) => {
                        conn.exec_bound(sql!(
                    ...
                    SerialieditsyncWorkspaceLocation::Ssh(ssh_project) => {
                        conn.exec_bound(sql!(
                            DELETE FROM toolchains WHERE workspace_id = ?1;
    ...
    pub(crate) async fn get_or_create_ssh_project(
        ...
        paths: Vec<String>,
        user: Option<String>,
    ) -> Result<SerialieditsyncSshProject> {
    ...
    query! {
        async fn get_ssh_project(host: String, port: Option<u16>, paths: String, user: Option<String>) -> Result<Option<SerialieditsyncSshProject>> {
    ...
    query! {
        async fn insert_ssh_project(host: String, port: Option<u16>, paths: String, user: Option<String>) -> Result<Option<SerialieditsyncSshProject>> {
    ...
    query! {
        fn ssh_projects() -> Result<Vec<SerialieditsyncSshProject>> {
            SELECT id, host, port, paths, user
            FROM ssh_projects
        }
    }
    ...
    query! {
        fn ssh_project(id: u64) -> Result<SerialieditsyncSshProject> {
            SELECT id, host, port, paths, user
            FROM ssh_projects
            WHERE id = ?
        }
    ...
    pub async fn recent_workspaces_on_disk(
        &self,
    ) -> Result<Vec<(WorkspaceId, SerialieditsyncWorkspaceLocation)>> {
        ...
        for (id, location, order, ssh_project_id) in self.recent_workspaces()? {
            ...
            {
                result.push((id, SerialieditsyncWorkspaceLocation::Local(location, order)));
            } else {
                delete_tasks.push(self.delete_workspace_by_id(id));
            }
        }
    ...
    pub async fn last_workspace(&self) -> Result<Option<SerialieditsyncWorkspaceLocation>> {
        Ok(self
    ...
    // session was closed (i.e. when Editsync was quit).
    ...
    pub fn last_session_workspace_locations(
    ...
    ) -> Result<Vec<SerialieditsyncWorkspaceLocation>> {
            ...
            self.session_workspaces(last_session_id.to_owned())?
        {
            if let Some(ssh_project_id) = ssh_project_id {
                ...
                && location.paths().iter().any(|path| path.is_dir())
            {
                let location = SerialieditsyncWorkspaceLocation::Local(location, order);
                workspaces.push((location, window_id.map(WindowId::from)));
            }
        }
    ...
    fn get_center_pane_group(&self, workspace_id: WorkspaceId) -> Result<SerialieditsyncPaneGroup> {
        Ok(self
            .get_pane_group(workspace_id, None)?
            .into_iter()
            .next()
            .unwrap_or_else(|| {
                SerialieditsyncPaneGroup::Pane(SerialieditsyncPane {
                    active: true,
                    children: vec![],
                    pinned_count: 0,
                })
    ...
    fn get_pane_group(
    ...
    ) -> Result<Vec<SerialieditsyncPaneGroup>> {
        type GroupKey = (Option<GroupId>, WorkspaceId);
        type GroupOrPane = (
            Option<GroupId>,
            Option<SerialieditsyncAxis>,
            Option<PaneId>,
            Option<bool>,
            Option<usize>,
            Option<String>,
        ...
        .map(|(group_id, axis, pane_id, active, pinned_count, flexes)| {
            ...
            if let Some((group_id, axis)) = group_id.zip(axis) {
                ...
                Ok(SerialieditsyncPaneGroup::Group {
                    axis,
                    children: self.get_pane_group(workspace_id, Some(group_id))?,
                    flexes,
                })
            } else if let Some((pane_id, active, pinned_count)) = maybe_pane {
                Ok(SerialieditsyncPaneGroup::Pane(SerialieditsyncPane::new(
        ...
        .filter(|pane_group| match pane_group {
            Ok(SerialieditsyncPaneGroup::Group { children, .. }) => !children.is_empty(),
            Ok(SerialieditsyncPaneGroup::Pane(pane)) => !pane.children.is_empty(),
            _ => true,
        })
        .collect::<Result<_>>()
    }
    ...
    fn save_pane_group(
        conn: &Connection,
        workspace_id: WorkspaceId,
        pane_group: &SerialieditsyncPaneGroup,
        parent: Option<(GroupId, usize)>,
    ) -> Result<()> {
        match pane_group {
            SerialieditsyncPaneGroup::Group {
                axis,
                children,
                flexes,
            } => {
            ...
            SerialieditsyncPaneGroup::Pane(pane) => {
    ...
    fn get_items(&self, pane_id: PaneId) -> Result<Vec<SerialieditsyncItem>> {
    ...
    query! {
        pub(crate) async fn set_window_open_status(workspace_id: WorkspaceId, bounds: SerialieditsyncWindowBounds, display: Uuid) -> Result<()> {
            UPDATE workspaces
            SET window_state = ?2,
                window_x = ?3,
                window_y = ?4,
...
mod tests {
    ...
    async fn test_workspace_id_stability() {
        ...
        let mut workspace_1 = SerialieditsyncWorkspace {
            id: WorkspaceId(1),
            location: SerialieditsyncWorkspaceLocation::from_local_paths(["/tmp", "/tmp2"]),
        ...
        let workspace_2 = SerialieditsyncWorkspace {
            id: WorkspaceId(2),
            location: SerialieditsyncWorkspaceLocation::from_local_paths(["/tmp"]),
            center_group: Default::default(),
            window_bounds: Default::default(),
            display: Default::default(),
            docks: Default::default(),
    ...
    fn group(axis: Axis, children: Vec<SerialieditsyncPaneGroup>) -> SerialieditsyncPaneGroup {
        SerialieditsyncPaneGroup::Group {
            axis: SerialieditsyncAxis(axis),
            flexes: None,
            children,
        }
    }
    ...
    async fn test_full_workspace_serialization() {
        ...
        let center_group = group(
            ...
            vec![
                group(
                    Axis::Vertical,
                    vec![
                        SerialieditsyncPaneGroup::Pane(SerialieditsyncPane::new(
                            vec![
                                SerialieditsyncItem::new("Terminal", 5, false, false),
                                SerialieditsyncItem::new("Terminal", 6, true, false),
                            ],
                            false,
                            0,
                        )),
                        SerialieditsyncPaneGroup::Pane(SerialieditsyncPane::new(
                            vec![
                                SerialieditsyncItem::new("Terminal", 7, true, false),
                                SerialieditsyncItem::new("Terminal", 8, false, false),
                            ],
                            false,
                            0,
                        )),
                ...
                SerialieditsyncPaneGroup::Pane(SerialieditsyncPane::new(
                    vec![
                        SerialieditsyncItem::new("Terminal", 9, false, false),
                        SerialieditsyncItem::new("Terminal", 10, true, false),
                    ],
                    false,
                    0,
                )),
        ...
        let workspace = SerialieditsyncWorkspace {
            id: WorkspaceId(5),
    ...
    async fn test_workspace_assignment() {
        ...
        let workspace_1 = SerialieditsyncWorkspace {
            id: WorkspaceId(1),
            location: SerialieditsyncWorkspaceLocation::Local(
                LocalPaths::new(["/tmp", "/tmp2"]),
        ...
        let mut workspace_2 = SerialieditsyncWorkspace {
            id: WorkspaceId(2),
            location: SerialieditsyncWorkspaceLocation::from_local_paths(["/tmp"]),
            center_group: Default::default(),
            window_bounds: Default::default(),
        ...
        let mut workspace_3 = SerialieditsyncWorkspace {
            id: WorkspaceId(3),
            location: SerialieditsyncWorkspaceLocation::Local(
                LocalPaths::new(["/tmp", "/tmp2"]),
                LocalPathsOrder::new([1, 0]),
        ...
        workspace_3.location =
            SerialieditsyncWorkspaceLocation::from_local_paths(["/tmp3", "/tmp4", "/tmp2"]);
        db.save_workspace(workspace_3.clone()).await;
    ...
    async fn test_session_workspaces() {
        ...
        let workspace_1 = SerialieditsyncWorkspace {
            id: WorkspaceId(1),
            location: SerialieditsyncWorkspaceLocation::from_local_paths(["/tmp1"]),
            center_group: Default::default(),
            window_bounds: Default::default(),
            display: Default::default(),
            docks: Default::default(),
        ...
        let workspace_2 = SerialieditsyncWorkspace {
            id: WorkspaceId(2),
            location: SerialieditsyncWorkspaceLocation::from_local_paths(["/tmp2"]),
            center_group: Default::default(),
            window_bounds: Default::default(),
            display: Default::default(),
            docks: Default::default(),
        ...
        let workspace_3 = SerialieditsyncWorkspace {
            id: WorkspaceId(3),
            location: SerialieditsyncWorkspaceLocation::from_local_paths(["/tmp3"]),
            center_group: Default::default(),
            window_bounds: Default::default(),
            display: Default::default(),
            docks: Default::default(),
        ...
        let workspace_4 = SerialieditsyncWorkspace {
            id: WorkspaceId(4),
            location: SerialieditsyncWorkspaceLocation::from_local_paths(["/tmp4"]),
            center_group: Default::default(),
            window_bounds: Default::default(),
            display: Default::default(),
            docks: Default::default(),
        ...
        let workspace_5 = SerialieditsyncWorkspace {
            id: WorkspaceId(5),
            location: SerialieditsyncWorkspaceLocation::Ssh(ssh_project.clone()),
            center_group: Default::default(),
            window_bounds: Default::default(),
        ...
        let workspace_6 = SerialieditsyncWorkspace {
            id: WorkspaceId(6),
            location: SerialieditsyncWorkspaceLocation::Local(
    ...
    fn default_workspace<P: AsRef<Path>>(
        workspace_id: &[P],
        center_group: &SerialieditsyncPaneGroup,
    ) -> SerialieditsyncWorkspace {
        SerialieditsyncWorkspace {
            id: WorkspaceId(4),
            location: SerialieditsyncWorkspaceLocation::from_local_paths(workspace_id),
    ...
    async fn test_last_session_workspace_locations() {
        ...
        assert_eq!(
            have[0],
            SerialieditsyncWorkspaceLocation::from_local_paths(&[dir4.path()])
        );
        assert_eq!(
            have[1],
            SerialieditsyncWorkspaceLocation::from_local_paths([dir3.path()])
        );
        assert_eq!(
            have[2],
            SerialieditsyncWorkspaceLocation::from_local_paths([dir2.path()])
        );
        assert_eq!(
            have[3],
            SerialieditsyncWorkspaceLocation::from_local_paths([dir1.path()])
        );
        assert_eq!(
            have[4],
            SerialieditsyncWorkspaceLocation::Local(
    ...
    async fn test_last_session_workspace_locations_ssh_projects() {
        ...
        .into_iter()
        .map(|(id, ssh_project, window_id)| SerialieditsyncWorkspace {
            id: WorkspaceId(id),
            location: SerialieditsyncWorkspaceLocation::Ssh(ssh_project),
            center_group: Default::default(),
            window_bounds: Default::default(),
            display: Default::default(),
            docks: Default::default(),
        ...
        assert_eq!(
            have[0],
            SerialieditsyncWorkspaceLocation::Ssh(ssh_projects[3].clone())
        );
        assert_eq!(
            have[1],
            SerialieditsyncWorkspaceLocation::Ssh(ssh_projects[2].clone())
        );
        assert_eq!(
            have[2],
            SerialieditsyncWorkspaceLocation::Ssh(ssh_projects[1].clone())
        );
        assert_eq!(
            have[3],
            SerialieditsyncWorkspaceLocation::Ssh(ssh_projects[0].clone())
        );
    }
            ...
            vec![
                group(
                    ...
                    vec![
                        SerialieditsyncPaneGroup::Pane(SerialieditsyncPane::new(
                            vec![
                                SerialieditsyncItem::new("Terminal", 1, false, false),
                                SerialieditsyncItem::new("Terminal", 2, true, false),
                            ],
                            false,
                            0,
                        )),
                        SerialieditsyncPaneGroup::Pane(SerialieditsyncPane::new(
                            vec![
                                SerialieditsyncItem::new("Terminal", 4, false, false),
                                SerialieditsyncItem::new("Terminal", 3, true, false),
                            ],
                            true,
                            0,
                        )),
                ...
                SerialieditsyncPaneGroup::Pane(SerialieditsyncPane::new(
                    vec![
                        SerialieditsyncItem::new("Terminal", 5, true, false),
                        SerialieditsyncItem::new("Terminal", 6, false, false),
                    ],
                    false,
                    0,
                )),
    ...
    async fn test_cleanup_panes() {
        ...
        workspace.center_group = group(
            ...
            vec![
                ...
                SerialieditsyncPaneGroup::Pane(SerialieditsyncPane::new(
                    vec![
                        SerialieditsyncItem::new("Terminal", 4, true, false),
                        SerialieditsyncItem::new("Terminal", 3, false, false),
                    ],
                    true,
                    0,
                )),
