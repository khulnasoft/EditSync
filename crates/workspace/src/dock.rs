...
pub struct Dock {
    ...
    pub(crate) serialieditsync_dock: Option<DockData>,
    resizeable: bool,
...
impl Dock {
    ...
    pub fn restore_state(&mut self, cx: &mut ViewContext<Self>) -> bool {
        if let Some(serialieditsync) = self.serialieditsync_dock.clone() {
            if let Some(active_panel) = serialieditsync.active_panel {
                if let Some(idx) = self.panel_index_for_persistent_name(active_panel.as_str(), cx) {
                    self.activate_panel(idx, cx);
                }
            }

            if serialieditsync.zoom {
                if let Some(panel) = self.active_panel() {
                    panel.set_zoomed(true, cx)
                }
            }
            self.set_open(serialieditsync.visible, cx);
            return true;
        }
        false
    }
