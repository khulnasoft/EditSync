...
pub use pane_group::*;
pub use persistence::{
    model::{ItemId, LocalPaths, SerialieditsyncWorkspaceLocation},
    WorkspaceDb, DB as WORKSPACE_DB,
};
use persistence::{
    model::{SerialieditsyncSshProject, SerialieditsyncWorkspace},
    SerialieditsyncWindowBounds, DB,
};
...
pub use ui;
use ui::prelude::*;
use util::{paths::SanitieditsyncPath, serde::default_true, ResultExt, TryFutureExt};
use uuid::Uuid;
...
static EDITSYNC_WINDOW_SIZE: LazyLock<Option<Size<Pixels>>> = LazyLock::new(|| {
    env::var("EDITSYNC_WINDOW_SIZE")
        .ok()
        .as_deref()
        .and_then(parse_pixel_size_env_var)
});

static EDITSYNC_WINDOW_POSITION: LazyLock<Option<Point<Pixels>>> = LazyLock::new(|| {
    env::var("EDITSYNC_WINDOW_POSITION")
        .ok()
        .as_deref()
...
pub fn register_serializable_item<I: SerializableItem>(cx: &mut AppContext) {
    let serialieditsync_item_kind = I::serialieditsync_item_kind();
    ...
    registry
        .descriptors_by_kind
        .insert(Arc::from(serialieditsync_item_kind), descriptor);
    registry
        .descriptors_by_type
...
pub struct Workspace {
    ...
    serialieditsync_ssh_project: Option<SerialieditsyncSshProject>,
...
impl Workspace {
    ...
    pub fn new(
                                ...
                                if let Some(database_id) = workspace_id {
                                    cx.background_executor()
                                        .spawn(DB.set_window_open_status(
                                            database_id,
                                            SerialieditsyncWindowBounds(window_bounds),
                                            display_uuid,
                                        ))
                                        .detach_and_log_err(cx);
                                }
        ...
        Workspace {
            ...
            serialieditsync_ssh_project: None,
        }
    }
    ...
    > {
        ...
        cx.spawn(|mut cx| async move {
            ...
            let serialieditsync_workspace: Option<SerialieditsyncWorkspace> =
                persistence::DB.workspace_for_roots(paths_to_open.as_slice());

            let workspace_location = serialieditsync_workspace
                .as_ref()
                .map(|ws| &ws.location)
                .and_then(|loc| match loc {
                    SerialieditsyncWorkspaceLocation::Local(paths, order) => {
            ...
            if let Some((paths, order)) = workspace_location {
                // todo: should probably move this logic to a method on the SerialieditsyncWorkspaceLocation
            ...
            let window = if let Some(window) = requesting_window {
                ...
                let (window_bounds, display) = if let Some(bounds) = window_bounds_override {
                    ...
                    let restorable_bounds = serialieditsync_workspace
                        .as_ref()
                ...
                // Use the serialieditsync workspace to construct the new window
                let mut options = cx.update(|cx| (app_state.build_window_options)(display, cx))?;
                options.window_bounds = window_bounds;
                let centered_layout = serialieditsync_workspace
                    .as_ref()
            ...
            let opened_items = window
                .update(&mut cx, |_workspace, cx| {
                    open_items(serialieditsync_workspace, project_paths, cx)
                })?
                .await
                .unwrap_or_default();
    ...
    pub fn serialieditsync_ssh_project(&self) -> Option<SerialieditsyncSshProject> {
    ...
    pub fn open_paths(
        ...
        cx.spawn(move |this, mut cx| async move {
            ...
            for abs_path in &abs_paths {
                ...
                let this = this.clone();
                let abs_path: Arc<Path> = SanitieditsyncPath::from(abs_path.clone()).into();
                let fs = fs.clone();
                let pane = pane.clone();
    ...
    async fn add_view_from_leader(
        ...
        let item = if let Some(existing_item) = existing_item {
            ...
            let Some(task) = task else {
                return Err(anyhow!(
                    "failed to construct view from leader (maybe from a different version of editsync?)"
                ));
            };
    ...
    fn serialize_workspace_internal(&self, cx: &mut WindowContext) -> Task<()> {
        ...
        fn serialize_pane_handle(pane_handle: &View<Pane>, cx: &WindowContext) -> SerialieditsyncPane {
                ...
                (
                    pane.items()
                        .filter_map(|handle| {
                            ...
                            Some(SerialieditsyncItem {
                                kind: Arc::from(handle.serialieditsync_item_kind()),
            ...
            SerialieditsyncPane::new(items, active, pinned_count)
        }

        fn build_serialieditsync_pane_group(
            pane_group: &Member,
            cx: &WindowContext,
        ) -> SerialieditsyncPaneGroup {
            match pane_group {
                Member::Axis(PaneAxis {
                    ...
                    children: members
                        .iter()
                        .map(|member| build_serialieditsync_pane_group(member, cx))
                        .collect::<Vec<_>>(),
                    flexes: Some(flexes.lock().clone()),
                },
                Member::Pane(pane_handle) => {
                    SerialieditsyncPaneGroup::Pane(serialize_pane_handle(pane_handle, cx))
                }
            }
        }

        fn build_serialieditsync_docks(this: &Workspace, cx: &mut WindowContext) -> DockStructure {
        ...
        let location = if let Some(ssh_project) = &self.serialieditsync_ssh_project {
        ...
        if let Some(location) = location {
            let center_group = build_serialieditsync_pane_group(&self.center.root, cx);
            let docks = build_serialieditsync_docks(self, cx);
            let window_bounds = Some(SerialieditsyncWindowBounds(cx.window_bounds()));
            let serialieditsync_workspace = SerialieditsyncWorkspace {
                id: database_id,
                location,
                center_group,
                window_bounds,
            ...
            return cx.spawn(|_| persistence::DB.save_workspace(serialieditsync_workspace));
        }
        Task::ready(())
    }
    ...
    pub(crate) fn load_workspace(
        serialieditsync_workspace: SerialieditsyncWorkspace,
        ...
        cx.spawn(|workspace, mut cx| async move {
            ...
            if let Some((group, active_pane, items)) = serialieditsync_workspace
                .center_group
                .deserialize(
                    &project,
                    serialieditsync_workspace.id,
            ...
            let mut all_deserialieditsync_items = Vec::default();
            cx.update(|cx| {
                for item in center_items.unwrap_or_default().into_iter().flatten() {
                    if let Some(serializable_item_handle) = item.to_serializable_item_handle(cx) {
                        item_ids_by_kind
                            .entry(serializable_item_handle.serialieditsync_item_kind())
                            .or_insert(Vec::new())
                    ...
                    all_deserialieditsync_items.push(item);
                }
            })?;
            ...
            workspace.update(&mut cx, |workspace, cx| {
                ...
                let docks = serialieditsync_workspace.docks;

                for (dock, serialieditsync_dock) in [
                ...
                {
                    dock.update(cx, |dock, cx| {
                        dock.serialieditsync_dock = Some(serialieditsync_dock.clone());
...
fn window_bounds_env_override() -> Option<Bounds<Pixels>> {
    EDITSYNC_WINDOW_POSITION
        .zip(*EDITSYNC_WINDOW_SIZE)
...
fn open_items(
    serialieditsync_workspace: Option<SerialieditsyncWorkspace>,
    mut project_paths_to_open: Vec<(PathBuf, Option<ProjectPath>)>,
    cx: &mut ViewContext<Workspace>,
) -> impl 'static + Future<Output = Result<Vec<Option<Result<Box<dyn ItemHandle>>>>>> {
    let restored_items = serialieditsync_workspace.map(|serialieditsync_workspace| {
...
fn notify_if_database_failed(workspace: WindowHandle<Workspace>, cx: &mut AsyncAppContext) {
    const REPORT_ISSUE_URL: &str = "https://github.com/khulnasoft/editsync/issues/new?assignees=&labels=admin+read%2Ctriage%2Cbug&projects=&template=1_bug_report.yml";
...
pub async fn last_opened_workspace_location() -> Option<SerialieditsyncWorkspaceLocation> {
                    ...
                    .update(&mut cx, |_, cx| {
                        let detail: SharedString = match err.error_code() {
                            ...
                            ErrorCode::UpgradeRequired => {
                                "Your are running an unsupported version of Editsync. Please update to continue.".into()
                            }
...
pub async fn get_any_active_workspace(
    ...
    activate_any_workspace_window(&mut cx).context("could not open editsync")
}
...
pub fn open_ssh_project(
    ...
    cx.spawn(|mut cx| async move {
        let (serialieditsync_ssh_project, workspace_id, serialieditsync_workspace) =
        ...
        cx.update_window(window.into(), |_, cx| {
            cx.replace_root_view(|cx| {
                ...
                workspace.set_serialieditsync_ssh_project(serialieditsync_ssh_project);
                workspace
            });
        })?;
        ...
        window
            .update(&mut cx, |_, cx| {
                ...
                open_items(serialieditsync_workspace, project_paths_to_open, cx)
            })?
            .await?;
...
fn serialize_ssh_project(
    ...
    Result<(
        SerialieditsyncSshProject,
        WorkspaceId,
        Option<SerialieditsyncWorkspace>,
    )>,
> {
    cx.background_executor().spawn(async move {
        let serialieditsync_ssh_project = persistence::DB
...
mod tests {
    ...
    async fn test_panels(cx: &mut gpui::TestAppContext) {
        ...
        workspace.update(cx, |workspace, cx| {
            ...
            assert!(!workspace.left_dock().read(cx).is_open());
            // The bottom dock is sieditsync based on the panel's default size,
