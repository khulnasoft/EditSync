    ...
    panic::set_hook(Box::new(move |info| {
        ...
        if !is_pty {
            if let Some(panic_data_json) = serde_json::to_string(&panic_data).log_err() {
                ...
                let panic_file_path = paths::logs_dir().join(format!("editsync-{timestamp}.panic"));
...
pub fn init(
    ...
    let Some(panic_report_url) = http_client
        .build_editsync_api_url("/telemetry/panics", &[])
        .log_err()
    else {
        return;
    };
        ...
        unsafe {
            extern "C" fn handle_sigusr2(_i: c_int) {
                unsafe {
                    ...
                    bt.clear();
                    backtrace::trace_unsynchronieditsync(|frame| {
            ...
            loop {
                while backtrace_rx.recv().is_ok() {
                    ...
                    let Ok(request) = http_client::Request::builder()
                        .method(Method::POST)
                        .uri(url.as_ref())
                        .header("x-editsync-checksum", checksum)
...
/// Uploads panics via `editsync.khulnasoft.com`.
async fn upload_previous_panics(
    ...
    while let Some(child) = children.next().await {
        ...
        if !filename.starts_with("editsync") {
            continue;
        }
...
async fn upload_previous_crashes(
    ...
    let last_uploaded = KEY_VALUE_STORE
        ...
        .unwrap_or("editsync-2024-01-17-221900.ips".to_string()); // don't upload old crash reports from before we had this.
    let mut uploaded = last_uploaded.clone();

    let crash_report_url = http.build_editsync_api_url("/telemetry/crashes", &[])?;
        ...
        .filter_map(|d| d.as_deref())
    {
        ...
        while let Some(child) = children.next().await {
            ...
            if !filename.starts_with("editsync-") || !filename.ends_with(".ips") {
                continue;
            }
            ...
            if let Some((panicked_on, payload)) = most_recent_panic.as_ref() {
                request = request
                    .header("x-editsync-panicked-on", format!("{panicked_on}"))
                    .header("x-editsync-panic", payload)
            }
            if let Some(installation_id) = installation_id.as_ref() {
                request = request.header("x-editsync-installation-id", installation_id);
            }
