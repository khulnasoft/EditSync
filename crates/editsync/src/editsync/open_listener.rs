...
use client::parse_editsync_link;
...
use workspace::{AppState, OpenOptions, SerialieditsyncWorkspaceLocation, Workspace};
...
impl OpenRequest {
    pub fn parse(urls: Vec<String>, cx: &AppContext) -> Result<Self> {
        ...
        for url in urls {
            if let Some(server_name) = url.strip_prefix("editsync-cli://") {
                this.cli_connection = Some(connect_to_cli(server_name)?);
            } else if let Some(file) = url.strip_prefix("file://") {
                this.parse_file_path(file)
            } else if let Some(file) = url.strip_prefix("editsync://file") {
                this.parse_file_path(file)
            } else if let Some(file) = url.strip_prefix("editsync://ssh") {
                let ssh_url = "ssh:/".to_string() + file;
                this.parse_ssh_file_path(&ssh_url, cx)?
            } else if url.starts_with("ssh://") {
                this.parse_ssh_file_path(&url, cx)?
            } else if let Some(request_path) = parse_editsync_link(&url, cx) {
    ...
    fn parse_request_path(&mut self, request_path: &str) -> Result<()> {
        ...
        Err(anyhow!("invalid editsync url: {}", request_path))
    }
}
...
pub fn listen_for_cli_connections(opener: OpenListener) -> Result<()> {
    ...
    let sock_path = paths::support_dir().join(format!("editsync-{}.sock", *RELEASE_CHANNEL_NAME));
...
async fn open_workspaces(
    ...
    let grouped_locations = if paths.is_empty() {
        ...
        vec![SerialieditsyncWorkspaceLocation::from_local_paths(
    ...
    if grouped_locations.is_empty() {
        ...
        for location in grouped_locations {
            match location {
                SerialieditsyncWorkspaceLocation::Local(workspace_paths, _) => {
...
pub async fn derive_paths_with_position(
    ...
    join_all(path_strings.into_iter().map(|path_str| async move {
        let canonicalieditsync = fs.canonicalize(Path::new(path_str.as_ref())).await;
        (path_str, canonicalieditsync)
    }))
    .await
    .into_iter()
    .map(|(original, canonicalieditsync)| match canonicalieditsync {
        Ok(canonicalieditsync) => PathWithPosition::from_path(canonicalieditsync),
...
mod tests {
    ...
    use crate::editsync::{open_listener::open_local_workspace, tests::init_test};
    ...
    async fn test_open_workspace_with_directory(cx: &mut TestAppContext) {
        ...
        // Now open a file inside that workspace, but tell Editsync to open a new window
    ...
    async fn test_open_workspace_with_nonexistent_files(cx: &mut TestAppContext) {
        ...
        // but tell Editsync to add it to the current workspace
        ...
        // but tell Editsync to NOT add it to the current workspace
