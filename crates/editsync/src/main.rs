...
use client::{parse_editsync_link, Client, ProxySettings, UserStore};
...
fn files_not_createad_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "Editsync failed to launch";
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut AppContext) {
    eprintln!(
        "Editsync failed to open a window: {e:?}. See https://editsync.khulnasoft.com/docs/linux for troubleshooting steps."
    );
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(|_cx| async move {
            ...
            let notification_id = "dev.editsync.Oops";
            proxy
                .add_notification(
                    notification_id,
                    Notification::new("Editsync failed to launch")
                        .body(Some(
                            format!(
                                "{e:?}. See https://editsync.khulnasoft.com/docs/linux for troubleshooting steps."
                            )
                            .as_str(),
                        ))
...
fn main() {
    ...
    let failed_single_instance_check =
        if *db::EDITSYNC_STATELESS || *release_channel::RELEASE_CHANNEL == ReleaseChannel::Dev {
            false
        } else {
            #[cfg(any(target_os = "linux", target_os = "freebsd"))]
            {
                crate::editsync::listen_for_cli_connections(open_listener.clone()).is_err()
            }

            #[cfg(target_os = "windows")]
            {
                !crate::editsync::windows_only_instance::check_single_instance()
            }

            #[cfg(target_os = "macos")]
            {
                use editsync::mac_only_instance::*;
                ensure_only_instance() != IsOnlyInstance::Yes
            }
        };
    if failed_single_instance_check {
        println!("editsync is already running");
        return;
    }

    let git_hosting_provider_registry = Arc::new(GitHostingProviderRegistry::new());
    let git_binary_path =
        if cfg!(target_os = "macos") && option_env!("EDITSYNC_BUNDLE").as_deref() == Some("true") {
    ...
    app.run(move |cx| {
        ...
        if let Some(build_sha) = option_env!("EDITSYNC_COMMIT_SHA") {
        ...
        #[cfg(any(target_os = "linux", target_os = "freebsd"))]
        crate::editsync::linux_prompts::init(cx);
                    ...
                    workspace.show_notification(id.clone(), cx, |cx| {
                        cx.new_view(|_| {
                            MessageNotification::new(format!("Invalid keymap file\n{error}"))
                                ...
                                .on_click(|cx| {
                                    cx.dispatch_action(editsync_actions::OpenKeymap.boxed_clone());
...
async fn authenticate(client: Arc<Client>, cx: &AsyncAppContext) -> Result<()> {
    if stdout_is_a_pty() {
        if *client::EDITSYNC_DEVELOPMENT_AUTH {
...
async fn restore_or_create_workspace(
    ...
    if let Some(locations) = restorable_workspace_locations(cx, &app_state).await {
        for location in locations {
            match location {
                SerialieditsyncWorkspaceLocation::Local(location, _) => {
                ...
                SerialieditsyncWorkspaceLocation::Ssh(ssh) => {
...
#[derive(Parser, Debug)]
#[command(name = "editsync", disable_version_flag = true)]
struct Args {
    ...
    ///
    /// URLs can either be `file://` or `editsync://` scheme, or relative to <https://editsync.khulnasoft.com>.
    paths_or_urls: Vec<String>,

    /// Instructs editsync to run as a dev server on this machine. (not implemented)
    #[arg(long)]
    ...
    match std::fs::canonicalize(Path::new(&arg)) {
        ...
        Err(error) => {
            if arg.starts_with("file://")
                || arg.starts_with("editsync-cli://")
                || arg.starts_with("ssh://")
                || parse_editsync_link(arg, cx).is_some()
            {
                Ok(arg.into())
            } else {
