...
use client::{editsync_urls, EDITSYNC_URL_SCHEME};
...
pub fn build_window_options(display_uuid: Option<Uuid>, cx: &mut AppContext) -> WindowOptions {
    ...
    let window_decorations = match std::env::var("EDITSYNC_WINDOW_DECORATIONS") {
...
pub fn initialize_workspace(
    ...
    feature_gate_editsync_pro_actions(cx);
}

fn feature_gate_editsync_pro_actions(cx: &mut AppContext) {
    let editsync_pro_actions = [TypeId::of::<OpenAccountSettings>()];

    CommandPaletteFilter::update_global(cx, |filter, _cx| {
        filter.hide_action_types(&editsync_pro_actions);
    });

    cx.observe_flag::<feature_flags::EditsyncPro, _>({
        move |is_enabled, cx| {
            CommandPaletteFilter::update_global(cx, |filter, _cx| {
                if is_enabled {
                    filter.show_action_types(editsync_pro_actions.iter());
                } else {
                    filter.hide_action_types(&editsync_pro_actions);
                }
            });
...
fn initialize_linux_file_watcher(cx: &mut ViewContext<Workspace>) {
    if let Err(e) = fs::linux_watcher::global(|_| {}) {
        let message = format!(
            ...
            This may be due to system-wide limits on inotify instances. For troubleshooting see: https://editsync.khulnasoft.com/docs/linux
            "#},
            e
        );
...
fn show_software_emulation_warning_if_needed(
    ...
    if specs.is_software_emulated && std::env::var("EDITSYNC_ALLOW_EMULATED_GPU").is_err() {
        let message = format!(
            db::indoc! {r#"
            Editsync uses Vulkan for rendering and requires a compatible GPU.

            Currently you are using a software emulated GPU ({}) which
            will result in awful performance.

            For troubleshooting see: https://editsync.khulnasoft.com/docs/linux
            Set EDITSYNC_ALLOW_EMULATED_GPU=1 env var to permanently override.
            "#},
            specs.device_name
        );
        let prompt = cx.prompt(
...
fn register_actions(
    ...
    workspace
        ...
        .register_action(|_, action: &OpenEditsyncUrl, cx| {
            OpenListener::global(cx).open_urls(vec![action.url.clone()])
        })
        .register_action(|_, action: &OpenBrowser, cx| cx.open_url(&action.url))
        .register_action(move |_, _: &editsync_actions::IncreaseBufferFontSize, cx| {
        ...
        .register_action(move |_, _: &editsync_actions::DecreaseBufferFontSize, cx| {
            theme::adjust_buffer_font_size(cx, |size| *size -= px(1.0))
        })
        .register_action(move |_, _: &editsync_actions::ResetBufferFontSize, cx| {
            theme::reset_buffer_font_size(cx)
        })
        .register_action(move |_, _: &editsync_actions::IncreaseUiFontSize, cx| {
            theme::adjust_ui_font_size(cx, |size| *size += px(1.0))
        })
        .register_action(move |_, _: &editsync_actions::DecreaseUiFontSize, cx| {
            theme::adjust_ui_font_size(cx, |size| *size -= px(1.0))
        })
        .register_action(move |_, _: &editsync_actions::ResetUiFontSize, cx| {
        ...
        .register_action(|_, _: &install_cli::RegisterEditsyncScheme, cx| {
            cx.spawn(|workspace, mut cx| async move {
                register_editsync_scheme(&cx).await?;
                workspace.update(&mut cx, |workspace, cx| {
                    struct RegisterEditsyncScheme;

                    workspace.show_toast(
                        Toast::new(
                            NotificationId::unique::<RegisterEditsyncScheme>(),
                            format!(
                                "editsync:// links will now open in {}.",
        ...
        .register_action(|workspace, _: &editsync_actions::OpenLicenses, cx| {
            open_bundled_file(
                workspace,
        ...
        .register_action(
            move |workspace: &mut Workspace,
                  _: &editsync_actions::OpenTelemetryLog,
                  cx: &mut ViewContext<Workspace>| {
        ...
        .register_action(
            move |workspace: &mut Workspace,
                  _: &editsync_actions::OpenDefaultKeymap,
                  cx: &mut ViewContext<Workspace>| {
                open_bundled_file(
                    workspace,
...
fn about(_: &mut Workspace, _: &editsync_actions::About, cx: &mut ViewContext<Workspace>) {
...
fn install_cli(_: &mut Workspace, _: &install_cli::Install, cx: &mut ViewContext<Workspace>) {
    const LINUX_PROMPT_DETAIL: &str = "If you installed Editsync from our official release add ~/.local/bin to your PATH.\n\nIf you installed Editsync from a different source like your package manager, then you may need to create an alias/symlink manually.\n\nDepending on your package manager, the CLI might be named editsyncitor, editsyncit, editsync-editor or something else.";
    ...
    cx.spawn(|workspace, mut cx| async move {
        ...
        workspace.update(&mut cx, |workspace, cx| {
            struct InstalledEditsyncCli;

            workspace.show_toast(
                Toast::new(
                    NotificationId::unique::<InstalledEditsyncCli>(),
                    format!(
                        "Installed `editsync` to {}. You can launch {} from your terminal.",
        ...
        register_editsync_scheme(&cx).await.log_err();
        Ok(())
    })
    .detach_and_prompt_err("Error installing editsync cli", cx, |_, _| None);
}
...
fn open_telemetry_log_file(workspace: &mut Workspace, cx: &mut ViewContext<Workspace>) {
    workspace.with_local_workspace(cx, move |workspace, cx| {
        ...
        cx.spawn(|workspace, mut cx| async move {
            ...
            let header = concat!(
                "// Editsync collects anonymous usage data to help us understand how people are using the app.\n",
...
async fn register_editsync_scheme(cx: &AsyncAppContext) -> anyhow::Result<()> {
    cx.update(|cx| cx.register_url_scheme(EDITSYNC_URL_SCHEME))?
        .await
}
...
mod tests {
    ...
    async fn test_window_edit_state_restoring_disabled(cx: &mut TestAppContext) {
        ...
        // Advance the clock to ensure that the item has been serialieditsync and dropped from the queue
    ...
    async fn test_window_edit_state_restoring_enabled(cx: &mut TestAppContext) {
        ...
        // Advance the clock to make sure the workspace is serialieditsync
        cx.executor().advance_clock(Duration::from_secs(1));
    ...
    fn test_bundled_settings_and_themes(cx: &mut AppContext) {
        cx.text_system()
            .add_fonts(vec![
                Assets
                    .load("fonts/plex-mono/EditsyncPlexMono-Regular.ttf")
                    .unwrap()
                    .unwrap(),
                Assets
                    .load("fonts/plex-sans/EditsyncPlexSans-Regular.ttf")
                    .unwrap()
                    .unwrap(),
            ])
            .unwrap();
