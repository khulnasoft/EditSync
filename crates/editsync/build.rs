...
fn main() {
    if cfg!(target_os = "macos") {
        ...
        println!("cargo:rerun-if-env-changed=EDITSYNC_BUNDLE");
        if std::env::var("EDITSYNC_BUNDLE").ok().as_deref() == Some("true") {
        ...
        // Weakly link ReplayKit to ensure Editsync can be used on macOS 10.15+.
        ...
        if output.status.success() {
            ...
            if let Ok(build_profile) = std::env::var("PROFILE") {
                if build_profile == "release" {
                    ...
                    println!(
                        "cargo:warning=Info: using '{git_sha}' hash for EDITSYNC_COMMIT_SHA env var"
                    );
                }
            }
        }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if let Some(explicit_rc_toolkit_path) = std::env::var("EDITSYNC_RC_TOOLKIT_PATH").ok() {
            res.set_toolkit_path(explicit_rc_toolkit_path.as_str());
        }
        res.set_icon(icon.to_str().unwrap());
        res.set("FileDescription", "Editsync");
        res.set("ProductName", "Editsync");
