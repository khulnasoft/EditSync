    ...
    fn render_rate_limit_notice(&self, cx: &mut ViewContext<Self>) -> impl IntoElement {
        Popover::new().child(
            v_flex()
                ...
                .child(Label::new(
                    "Try Editsync Pro for higher limits, a wider range of models, and more.",
                ))
                .child(
                    h_flex()
                        .justify_between()
                                ...
                                .child(Button::new("more-info", "More Info").on_click(
                                    |_event, cx| {
                                        cx.dispatch_action(Box::new(
                                            editsync_actions::OpenAccountSettings,
                                        ))
                                    },
                                )),
                        ),
...
pub enum PromptEditorEvent {
    ...
    CancelRequested,
    DismissRequested,
    Resieditsync { height_in_lines: u8 },
}
            ...
            PromptEditorMode::Terminal {
                ...
                if height_in_lines != *current_height {
                    *current_height = height_in_lines;
                    cx.emit(PromptEditorEvent::Resieditsync { height_in_lines });
                }
            }
