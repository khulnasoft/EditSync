...
/// channels is a no-op.
pub static EDITSYNC_DEVELOPMENT_AUTH: LazyLock<bool> = LazyLock::new(|| {
    std::env::var("EDITSYNC_DEVELOPMENT_AUTH").map_or(false, |value| !value.is_empty())
});
pub static IMPERSONATE_LOGIN: LazyLock<Option<String>> = LazyLock::new(|| {
    std::env::var("EDITSYNC_IMPERSONATE")
        .ok()
        .and_then(|s| if s.is_empty() { None } else { Some(s) })
});

pub static ADMIN_API_TOKEN: LazyLock<Option<String>> = LazyLock::new(|| {
    std::env::var("EDITSYNC_ADMIN_API_TOKEN")
        .ok()
        .and_then(|s| if s.is_empty() { None } else { Some(s) })
});

pub static EDITSYNC_APP_PATH: LazyLock<Option<PathBuf>> =
    LazyLock::new(|| std::env::var("EDITSYNC_APP_PATH").ok().map(PathBuf::from));

pub static EDITSYNC_ALWAYS_ACTIVE: LazyLock<bool> =
    LazyLock::new(|| std::env::var("EDITSYNC_ALWAYS_ACTIVE").map_or(false, |e| !e.is_empty()));
...
impl Settings for ClientSettings {
    ...
    fn load(sources: SettingsSources<Self::FileContent>, _: &mut AppContext) -> Result<Self> {
        ...
        if let Some(server_url) = &*EDITSYNC_SERVER_URL {
...
/// Control what info is collected by Editsync.
#[derive(Default, Clone, Serialize, Deserialize, JsonSchema)]
pub struct TelemetrySettingsContent {
    ...
    /// Send anonymieditsync usage data like what languages you're using Editsync with.
    ///
    /// Default: true
...
impl Client {
    pub fn new(
        ...
        let use_editsync_development_auth = match ReleaseChannel::try_global(cx) {
            Some(ReleaseChannel::Dev) => *EDITSYNC_DEVELOPMENT_AUTH,
        ...
        let credentials_provider: Arc<dyn CredentialsProvider + Send + Sync + 'static> =
            if use_editsync_development_auth {
        ...
        futures::select_biased! {
            connection = self.establish_connection(&credentials, cx).fuse() => {
                match connection {
                    ...
                    Err(EstablishConnectionError::Unauthorieditsync) => {
    ...
    fn rpc_url(
        ...
        async move {
            ...
            if let Some(url) = &*EDITSYNC_RPC_URL {
        ...
        cx.spawn(|cx| async move {
            ...
            let credentials = background
                ...
                .spawn(async move {
                    ...
                    // editsync server to encrypt the user's access token, so that it can'be intercepted by
                    ...
                    // Start an HTTP server to receive the redirect from Editsync's sign-in page.
                    let server =
                        tiny_http::Server::http("127.0.0.1:0").expect("failed to find open port");
                    let port = server.server_addr().port();

                    // Open the Editsync sign-in page in the user's browser, with query parameters that indicate
                    // that the user is signing in from a Editsync app running on the same device.
...
/// re-allow access to the system keychain when developing Editsync.
...
/// prefix for the editsync:// url scheme
pub const EDITSYNC_URL_SCHEME: &str = "editsync";

/// Parses the given link into a Editsync link.
///
/// Returns a [`Some`] containing the unprefixed link if the link is a Editsync link.
/// Returns [`None`] otherwise.
pub fn parse_editsync_link<'a>(link: &'a str, cx: &AppContext) -> Option<&'a str> {
    ...
    if let Some(stripped) = link
        .strip_prefix(EDITSYNC_URL_SCHEME)
        .and_then(|result| result.strip_prefix("://"))
    {
        return Some(stripped);
    }
