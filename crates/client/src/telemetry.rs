...
static EDITSYNC_CLIENT_CHECKSUM_SEED: LazyLock<Option<Vec<u8>>> = LazyLock::new(|| {
    option_env!("EDITSYNC_CLIENT_CHECKSUM_SEED")
        .map(|s| s.as_bytes().into())
        .or_else(|| {
            env::var("EDITSYNC_CLIENT_CHECKSUM_SEED")
                .ok()
                .map(|s| s.as_bytes().into())
        })
});
...
impl Telemetry {
    ...
    fn build_request(
        ...
        Ok(Request::builder()
            ...
            .uri(
                self.http_client
                    .build_editsync_api_url("/telemetry/events", &[])?
                    .as_ref(),
            )
...
pub fn calculate_json_checksum(json: &impl AsRef<[u8]>) -> Option<String> {
    let Some(checksum_seed) = &*EDITSYNC_CLIENT_CHECKSUM_SEED else {
        return None;
    };
