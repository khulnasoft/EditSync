...
/// Language server protocol RPC request response message before it is deserialieditsync into a concrete type.
#[derive(Serialize, Deserialize)]
...
/// Language server RPC notification message before it is deserialieditsync into a concrete type.
#[derive(Debug, Clone, Deserialize)]
                        ...
                        completion_item: Some(CompletionItemCapability {
                            ...
                            resolve_support: Some(CompletionItemCapabilityResolveSupport {
                                properties: vec![
                                    ...
                                    // NB: Do not have this resolved, otherwise Editsync becomes slow to complete things
                                    // "textEdit".to_string(),
                                ],
                            }),
                            ...
                            .spawn(async move {
                                let response = match result {
                                    Ok(response) => match serde_json::from_str(&response) {
                                        Ok(deserialieditsync) => Ok(deserialieditsync),
                                        Err(error) => {
