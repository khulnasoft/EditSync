...
impl HttpClientWithUrl {
    ...
    /// Builds a Editsync API URL using the given path.
    pub fn build_editsync_api_url(&self, path: &str, query: &[(&str, &str)]) -> Result<Url> {
    ...
    /// Builds a Editsync LLM URL using the given path.
    pub fn build_editsync_llm_url(&self, path: &str, query: &[(&str, &str)]) -> Result<Url> {
        let base_url = self.base_url();
        let base_api_url = match base_url.as_ref() {
