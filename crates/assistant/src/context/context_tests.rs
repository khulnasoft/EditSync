...
async fn test_workflow_step_parsing(cx: &mut TestAppContext) {
    ...
    let serialieditsync_context = context.read_with(cx, |context, cx| context.serialize(cx));
    let deserialieditsync_context = cx.new_model(|cx| {
        Context::deserialize(
            serialieditsync_context,
            Default::default(),
            registry.clone(),
            prompt_builder.clone(),
            Arc::new(SlashCommandWorkingSet::default()),
...
async fn test_serialization(cx: &mut TestAppContext) {
    ...
    let deserialieditsync_buffer =
        deserialieditsync_context.read_with(cx, |context, _| context.buffer.clone());
    assert_eq!(
        deserialieditsync_buffer.read_with(cx, |buffer, _| buffer.text()),
        "a\nb\nc\n"
    );
    assert_eq!(
        cx.read(|cx| messages(&deserialieditsync_context, cx)),
        [
            (message_0, Role::User, 0..2),
            (message_1.id, Role::Assistant, 2..6),
            (message_2.id, Role::System, 6..6),
