...
fn init_panic_hook() {
    std::panic::set_hook(Box::new(|info| {
        ...
        let panic_data = telemetry_events::Panic {
            ...
            app_version: format!(
                "remote-server-{}",
                option_env!("EDITSYNC_COMMIT_SHA").unwrap_or(&env!("EDITSYNC_PKG_VERSION"))
            ),
...
pub fn execute_run(
    ...
    gpui::App::headless().run(move |cx| {
        settings::init(cx);
        let app_version = AppVersion::init(env!("EDITSYNC_PKG_VERSION"));
...
fn cleanup_old_binaries() -> Result<()> {
    ...
    let prefix = format!("editsync-remote-server-{}-", release_channel);
...
fn is_new_version(version: &str) -> bool {
    SemanticVersion::from_str(version)
        .ok()
        .zip(SemanticVersion::from_str(env!("EDITSYNC_PKG_VERSION")).ok())
