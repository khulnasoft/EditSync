...
impl HeadlessProject {
    ...
    pub async fn handle_add_worktree(
        ...
        let canonicalieditsync = match fs.canonicalize(&path).await {
            Ok(path) => path,
            Err(e) => {
                let mut parent = path
                    .parent()
        ...
        let worktree = this
            .update(&mut cx.clone(), |this, _| {
                Worktree::local(
                    Arc::from(canonicalieditsync.as_path()),
        ...
        let response = this.update(&mut cx, |_, cx| {
            worktree.update(cx, |worktree, _| proto::AddWorktreeResponse {
                ...
                canonicalieditsync_path: canonicalieditsync.to_string_lossy().to_string(),
            })
        })?;
